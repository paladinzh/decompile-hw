...
public class HealthOpenSDK extends HealthOpenSDKCommon {
    ...
    private class TrackLocalToRemoteProxy extends ITrackDataReport.Stub {
        ...
        public TrackLocalToRemoteProxy(ICommonReport cb, IExecuteResult resultCb) {
            this.reportCb = cb;
            this.executeCb = resultCb;
        }
    ...
    public boolean registerTrackingReport(ICommonReport report, IExecuteResult cb) {
        Log.d(TAG, "registerTrackingReport:" + report + " at:" + System.currentTimeMillis());
        TrackLocalToRemoteProxy tmp = getTrackProxyFromList(report);
        if (this.mHandler != null) {
            if (tmp != null) {
                Log.w(TAG, "Already register same TrackingReport");
            } else {
                Message msg = this.mHandler.obtainMessage(102);
                msg.obj = new TrackLocalToRemoteProxy(report, cb);
                this.mHandler.sendMessage(msg);
                return true;
            }
        }
    ...
    private boolean registerTrackCallbackInter(TrackLocalToRemoteProxy report) {
        ...
        try {
            ...
            Log.w(TAG, "registerTrackCallbackInter : RemoteEx" + e.getMessage());
            return false;
        }
    }

    public void unRegisterTrackingReport(ICommonReport report) {
        Log.d(TAG, "unRegisterTrackingReport " + report);
        TrackLocalToRemoteProxy localCb = getTrackProxyFromList(report);
        if (localCb != null) {
            if (this.mITrackSportManager != null) {
                try {
                    this.mITrackSportManager.unRegisterDataCallback(localCb);
                    this.mContext.unbindService(this.myTrackConnection);
                    synchronized (this.mTrackReportList) {
                        this.mTrackReportList.remove(localCb);
                    }
                } catch (RemoteException e) {
    ...
    public int executeRemoteCommand(int cmd, IExecuteResult resultCb) {
        switch (cmd) {
            case 100:
                try {
    ...
    public int getTrackingState() {
        ...
        int state = 0;
        try {
            state = this.mITrackSportManager.isTrackWorking();
        } catch (RemoteException e) {
    ...
    private TrackLocalToRemoteProxy getTrackProxyFromList(ICommonReport cb) {
        synchronized (this.mTrackReportList) {
            int i = 0;
            while (i < this.mTrackReportList.size()) {
                if (((TrackLocalToRemoteProxy) this.mTrackReportList.get(i)).reportCb != cb) {
                    i++;
                } else {
                    TrackLocalToRemoteProxy trackLocalToRemoteProxy = (TrackLocalToRemoteProxy) this.mTrackReportList.get(i);
                    return trackLocalToRemoteProxy;
                }
            }
            return null;
        }
    ...
    private void cleanTrackProxyList() {

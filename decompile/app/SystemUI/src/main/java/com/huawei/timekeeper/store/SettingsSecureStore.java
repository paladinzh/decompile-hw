...
public class SettingsSecureStore extends Store {
    ...
    public boolean remove(Context context, int userHandle, String name) {
        if (-10000 == userHandle) {
            return Secure.putString(context.getContentResolver(), name, BuildConfig.FLAVOR);
        }
        return Secure.putStringForUser(context.getContentResolver(), name, BuildConfig.FLAVOR, userHandle);
    }
}

...
public class SearchPanelCircleView extends FrameLayout {
    ...
    public void setCircleSize(float circleSize, boolean animated, Runnable endRunnable, int startDelay, Interpolator interpolator) {
        HwLog.d("SearchPanelCircleView", "enter setCircleSize(circleSize:" + circleSize + ", animated:" + animated + ", startDelay:" + startDelay + ", interpolator:" + interpolator + ")");
        ...
        if (animated || animationPending || animatingOut) {
            ...
            if (isAnimating) {
                if (!SystemUiUtil.isFloatequal(circleSize, this.mCircleAnimationEndValue)) {
    ...
    private void updateLogo() {
        ...
        if (exitAnimationRunning) {
            ...
            this.mLogo.setAlpha(Math.max(((1.0f - t) - 0.5f) * 2.0f, 0.0f));
        }
    ...
    public void startAbortAnimation(Runnable endRunnable) {
        if (this.mAnimatingOut || this.mCircleHidden) {
            if (endRunnable != null) {
                endRunnable.run();
            }
            return;
        ...
        animate().alpha(0.0f).setDuration(300).setStartDelay(0).setInterpolator(PhoneStatusBarUtils.ALPHA_OUT).start();
    ...
    private void performExitFadeOutAnimation(int startDelay, int duration, final Runnable endRunnable) {
        ...
        this.mFadeOutAnimator.addUpdateListener(new AnimatorUpdateListener() {
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                float logoValue = PhoneStatusBarUtils.ALPHA_OUT.getInterpolation(1.0f - (animatedFraction > 0.5f ? 1.0f : animatedFraction / 0.5f));
                if (animatedFraction < 0.2f) {
                    backgroundValue = 0.0f;
                } else {
                    backgroundValue = PhoneStatusBarUtils.ALPHA_OUT.getInterpolation((animatedFraction - 0.2f) / 0.8f);
                }
    ...
    private void addRipple() {
        if (this.mRipples.size() <= 1) {
            ...
            new Ripple((((float) this.mStaticRect.left) * (1.0f - xInterpolation)) + (((float) this.mStaticRect.right) * xInterpolation), (((float) this.mStaticRect.top) * (1.0f - yInterpolation)) + (((float) this.mStaticRect.bottom) * yInterpolation), Math.max(this.mCircleSize, ((float) this.mCircleMinSize) * 1.25f) * 0.75f).start();
        }
    }

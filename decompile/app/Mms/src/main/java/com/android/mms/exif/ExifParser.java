    ...
    private ExifTag readTag() throws IOException, ExifInvalidFormatException {
        ...
        if (numOfComp > 2147483647L) {
            ...
            if (dataSize > 4) {
                ...
                long skip = this.mTiffStream.skip((long) (4 - dataSize));
                if (skip != ((long) (4 - dataSize))) {
                    Log.w("ExifParser", "ExifTag readTag() skip: " + skip);
                }
                tag.setOffset(this.mTiffStream.getReadByteCount() - 4);
            }
            return tag;
        } else {
            Log.w("ExifParser", String.format("Tag %04x: Invalid data type %d", new Object[]{Short.valueOf(tagId), Short.valueOf(dataFormat)}));
            long skipValue = this.mTiffStream.skip(4);
            if (skipValue != 4) {
                Log.w("ExifParser", "readTag() mTiffStream.skip(4) failed, skip value = " + skipValue);
            }
            return null;
        }
    }

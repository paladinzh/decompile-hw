...
public final class ActionIcon implements Parcelable {
    ...
    public static ActionIcon createWithResource(Context context, int resId) {
        if (context != null) {
            return createWithResource(context.getPackageName(), resId);
        }
        throw new IllegalArgumentException("Context must not be null.");
    }

    public static ActionIcon createWithResource(String resPackage, int resId) {
        if (resPackage != null) {
            ActionIcon rep = new ActionIcon(2);
            rep.mInt1 = resId;
    ...
    public static ActionIcon createWithBitmap(Bitmap bits) {
        if (bits != null) {
            ActionIcon rep = new ActionIcon(1);
            rep.setBitmap(bits);
            return rep;
        }

...
public class ViewPager extends ViewGroup {
    ...
    private static final Interpolator sInterpolator = new Interpolator() {
        public float getInterpolation(float t) {
            t -= ContentUtil.FONT_SIZE_NORMAL;
            return ((((t * t) * t) * t) * t) + ContentUtil.FONT_SIZE_NORMAL;
        }
    };
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(ContentUtil.FONT_SIZE_NORMAL, (((float) Math.abs(dx)) * ContentUtil.FONT_SIZE_NORMAL) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(Math.abs(distance / ((float) velocity)) * 1000.0f) * 4;
        } else {
            duration = (int) ((ContentUtil.FONT_SIZE_NORMAL + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + -1 ? (curItem.offset + curItem.widthFactor) - ContentUtil.FONT_SIZE_NORMAL : Float.MAX_VALUE;
        int i = curIndex - 1;
        while (i >= 0) {
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N - 1) {
                this.mLastOffset = (ii.widthFactor + offset) - ContentUtil.FONT_SIZE_NORMAL;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i++;
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == 1 && this.mAdapter != null && this.mAdapter.getCount() > 1)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                height = (getHeight() - getPaddingTop()) - getPaddingBottom();
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + ContentUtil.FONT_SIZE_NORMAL)) * ((float) width));

...
public class ViewPager extends ViewGroup {
    ...
    class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            if (event.getEventType() == FragmentTransaction.TRANSIT_ENTER_MASK && ViewPager.this.mAdapter != null) {
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(WMElement.CAMERASIZEVALUE1B1, (((float) Math.abs(dx)) * WMElement.CAMERASIZEVALUE1B1) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(Math.abs(distance / ((float) velocity)) * 1000.0f) * 4;
        } else {
            duration = (int) ((WMElement.CAMERASIZEVALUE1B1 + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + -1 ? (curItem.offset + curItem.widthFactor) - WMElement.CAMERASIZEVALUE1B1 : Float.MAX_VALUE;
        int i = curIndex - 1;
        while (i >= 0) {
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N - 1) {
                this.mLastOffset = (ii.widthFactor + offset) - WMElement.CAMERASIZEVALUE1B1;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i++;
        ...
        for (i = 0; i < size; i++) {
            ...
            if (child.getVisibility() != 8) {
                ...
                if (lp != null && lp.isDecor) {
                    ...
                    int widthMode = com.fyusion.sdk.viewer.internal.request.target.Target.SIZE_ORIGINAL;
                    int heightMode = com.fyusion.sdk.viewer.internal.request.target.Target.SIZE_ORIGINAL;
        ...
        switch (action) {
            ...
            case 2:
                ...
                if (activePointerId != -1) {
                    ...
                    if (pointerIndex == -1) {
                        Log.e("ViewPager", "pointerIndex out of range, pointerIndex = -1");
                        return true;
                    }
        ...
        switch (ev.getAction() & 255) {
            ...
            case 1:
                if (this.mIsBeingDragged) {
                    ...
                    Log.e("ViewPager", "activePointerIndex out of range, activePointerIndex = -1");
                    return true;
                }
                break;
            case 2:
                ...
                if (this.mIsBeingDragged) {
                    activePointerIndex = MotionEventCompat.findPointerIndex(ev, this.mActivePointerId);
                    if (activePointerIndex != -1) {
                        needsInvalidate = performDrag(MotionEventCompat.getX(ev, activePointerIndex));
                        break;
                    }
                    Log.e("ViewPager", "activePointerIndex out of range, activePointerIndex = -1");
                    return true;
            ...
            case 5:
                ...
                Log.e("ViewPager", "index out of range, index = -1");
                return true;
            case 6:
                onSecondaryPointerUp(ev);
        ...
        if (scrollX < leftBound) {
            if (leftAbsolute) {
                ...
                if (this.mHwViewPager != null) {
                    ...
                    z = this.mLeftEdge.onPull(Math.abs(over) / ((float) width));
                }
            }
            ...
            if (rightAbsolute) {
                ...
                if (this.mHwViewPager != null) {
                    ...
                    z = this.mRightEdge.onPull(Math.abs(over) / ((float) width));
                }
            }
            scrollX = this.mHwViewPager != null ? this.mHwViewPager.scrollEdgeBound(false, oldScrollX, deltaX, rightBound) : rightBound;
        }
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == 1 && this.mAdapter != null && this.mAdapter.getCount() > 1)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                height = (getHeight() - getPaddingTop()) - getPaddingBottom();
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + WMElement.CAMERASIZEVALUE1B1)) * ((float) width));
    ...
    public boolean dispatchPopulateAccessibilityEvent(AccessibilityEvent event) {
        if (event.getEventType() == FragmentTransaction.TRANSIT_ENTER_MASK) {

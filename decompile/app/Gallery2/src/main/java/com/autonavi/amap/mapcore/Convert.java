...
public class Convert {
    ...
    public static byte[] convertShort(int i) {
        return new byte[]{(byte) ((byte) (i & 255)), (byte) ((byte) ((i >> 8) & 255))};
    }
    ...
    public static String getString(byte[] bArr, int i, int i2) {
        try {
            return new String(bArr, i, i2, XmlUtils.INPUT_ENCODING);
        } catch (Exception e) {
            return "";
        }
    }
    ...
    public static byte[] get1BString(String str) {
        try {
            ...
            byteArrayOutputStream.write(new byte[]{(byte) ((byte) str.getBytes(XmlUtils.INPUT_ENCODING).length)});
    ...
    public static byte[] getDouble(double d) {
        ...
        for (int i = 0; i < 8; i++) {
            bArr[7 - i] = (byte) ((byte) Integer.parseInt(toHexString.substring(i * 2, (i * 2) + 2), 16));
        }
        return bArr;
    }
    ...
    public static void convert1bString(byte[] bArr, int i, ConvertString convertString) {
        try {
            ...
            convertString.value = new String(bArr, i + 1, convertString.byteLength, XmlUtils.INPUT_ENCODING);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void convert2bString(byte[] bArr, int i, ConvertString convertString) {
        try {
            ...
            convertString.value = new String(bArr, i + 2, convertString.byteLength, XmlUtils.INPUT_ENCODING);
        } catch (Exception e) {
            e.printStackTrace();
            convertString.byteLength = 0;
            convertString.value = "";

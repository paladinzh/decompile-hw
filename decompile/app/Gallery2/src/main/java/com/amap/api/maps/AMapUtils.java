...
public class AMapUtils {
    ...
    static class a extends Thread {
        ...
        public void run() {
            if (this.b != null) {
                try {
                    fa.a(this.b, new com.amap.api.mapcore.util.fh.a(this.a, "4.1.2", g.d).a(new String[]{"com.amap.api.maps"}).a());
                    interrupt();
                } catch (ex e) {
                    e.printStackTrace();
                }
    ...
    public static float calculateArea(LatLng latLng, LatLng latLng2) {
        ...
        double sin = Math.sin((latLng.latitude * 3.141592653589793d) / VirtualEarthProjection.MaxLongitude) - Math.sin((latLng2.latitude * 3.141592653589793d) / VirtualEarthProjection.MaxLongitude);
        double d = (latLng2.longitude - latLng.longitude) / 360.0d;
        if (d < 0.0d) {
            d += WeightedLatLng.DEFAULT_INTENSITY;
        }
        return (float) (d * (sin * (VirtualEarthProjection.EarthCircumferenceInMeters * 6378137.0d)));
    }
    ...
    private static String a(NaviPara naviPara, Context context) {
        return String.format("androidamap://navi?sourceApplication=%s&lat=%f&lon=%f&dev=0&style=%d", new Object[]{ey.b(context), Double.valueOf(naviPara.getTargetPoint().latitude), Double.valueOf(naviPara.getTargetPoint().longitude), Integer.valueOf(naviPara.getNaviStyle())});
    }

    private static String b(RoutePara routePara, Context context, int i) {
        String format = String.format("androidamap://route?sourceApplication=%s&slat=%f&slon=%f&sname=%s&dlat=%f&dlon=%f&dname=%s&dev=0&t=%d", new Object[]{ey.b(context), Double.valueOf(routePara.getStartPoint().latitude), Double.valueOf(routePara.getStartPoint().longitude), routePara.getStartName(), Double.valueOf(routePara.getEndPoint().latitude), Double.valueOf(routePara.getEndPoint().longitude), routePara.getEndName(), Integer.valueOf(i)});
        if (i == 1) {
    ...
    private static String a(PoiPara poiPara, Context context) {
        String format = String.format("androidamap://arroundpoi?sourceApplication=%s&keywords=%s&dev=0", new Object[]{ey.b(context), poiPara.getKeywords()});

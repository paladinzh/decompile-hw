...
public abstract class BasePaintState extends BaseMosaicState {
    ...
    public BasePaintState(Context context, ViewGroup layout, BaseEditorView editorView) {
        super(context, layout, editorView);
        this.mDefaultActionInfo = new ActionInfo(context.getString(getActionNameId()), Action.BACK, null, true);
        this.mRedoUndoActionInfo = new ActionInfo(context.getString(getActionNameId()), Action.BACK, null, Action.REDO, Action.UNDO, true);
    ...
    public void show() {
        ...
        if (bmp != null) {
            this.mFilterMosaicRepresentation.setBounds(new Rect(0, 0, bmp.getWidth(), bmp.getHeight()));
        }
    ...
    public void onStrokeDataChange(FilterMosaicRepresentation representation) {
        boolean canUndo = representation.canUndo();
        boolean canRedo = representation.canRedo();
        if (canUndo || canRedo) {
        ...
        changeActionBar();
        super.onStrokeDataChange(representation);
    }

    public void onStrokeDataCommit(FilterMosaicRepresentation representation) {
        onStrokeDataChange(representation);
    }

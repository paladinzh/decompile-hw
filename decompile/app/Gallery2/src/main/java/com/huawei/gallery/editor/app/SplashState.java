...
import com.huawei.gallery.editor.ui.SplashUIController.SplashListener;

public class SplashState extends BasePaintState implements SplashListener {
    ...
    public SplashState(Context context, ViewGroup layout, BaseEditorView editorView) {
        super(context, layout, editorView);
        for (FilterRepresentation rep : this.mEditorView.getEditorManager().getSplash()) {
            if (rep instanceof FilterSplashRepresentation) {
                this.mFilterMosaicRepresentation = (FilterSplashRepresentation) rep;
            }
        }
    }
    ...
    public void show() {
        super.show();
        ContextedUtils.showToastQuickly(this.mContext, (int) R.string.add_splash_tip, 1);
    }

    protected int getActionNameId() {
        return R.string.simple_editor_splash;
    }

    public int getColor(int x, int y) {
        Bitmap bitmap = getImage().getCacheLatestBitmap();
        if (bitmap == null) {
            bitmap = getImage().getOriginalBitmapLarge();
        }
        if (bitmap == null || this.mSplashUIController == null || !this.mSplashUIController.isPointValid(x, y)) {
            return 0;
        }

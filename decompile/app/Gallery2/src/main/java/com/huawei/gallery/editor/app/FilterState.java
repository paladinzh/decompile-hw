...
public class FilterState extends EditorState {
    ...
    public FilterState(Context context, ViewGroup layout, BaseEditorView editorView) {
        super(context, layout, editorView);
        this.mActionInfo = new ActionInfo(context.getResources().getString(R.string.simple_editor_filter), Action.BACK, null, false);
    }
    ...
    protected EditorUIController createUIController() {
        ...
        SparseArray<CategoryAdapter> filtersAdpater = this.mEditorView.getEditorManager().getFiltersAdpater();
        if (filtersAdpater != null) {
            this.mFilterUIController.setAdapter(filtersAdpater);
        }

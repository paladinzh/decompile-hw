...
public abstract class SkinState extends EditorState implements SkinUIListener {
    ...
    private SkinUIController mSkinUIController;
    ...
    protected abstract BaseViewAdapter createAdapter();

    protected abstract FaceEditorStep createEditorStep();

    protected abstract SkinUIController createSkinUIController();

    public SkinState(Context context, ViewGroup layout, BaseEditorView editorView) {
        super(context, layout, editorView);
        this.mActionInfo = new ActionInfo(context.getResources().getString(R.string.filtershow_title_beauty), Action.BACK, null, false);
    }
    ...
    protected EditorUIController createUIController() {
        this.mSkinUIController = createSkinUIController();
        this.mAdapter = createAdapter();
        this.mSkinUIController.setAdpater(this.mAdapter);
        return this.mSkinUIController;
    }

    public void show() {
        this.mFaceEditorStep = createEditorStep();
        if (this.mSkinUIController != null) {
            this.mSkinUIController.setEditorStep(this.mFaceEditorStep);
        }
        super.show();
    }
    ...
    public void executeAction(Action action) {
        switch (-getcom-huawei-gallery-actionbar-ActionSwitchesValues()[action.ordinal()]) {
            case 1:
                getSimpleEditorManager().pushEditorStep(this.mFaceEditorStep);
                return;
            default:
                return;
        }

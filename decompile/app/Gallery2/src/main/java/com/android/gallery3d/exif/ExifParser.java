...
class ExifParser {
    private static final Charset PARSER_CHARSET = Charset.forName(XmlUtils.INPUT_ENCODING);
    ...
    protected int next() throws IOException, ExifInvalidFormatException {
        ...
        if (offset == endOfTags) {
            ...
            if (this.mIfdType == 0) {
                ...
                if (offsetSize < 4) {
                    GalleryLog.w("ExifParser", "Invalid size of link to next IFD: " + offsetSize);
                } else {
                    ifdOffset = readUnsignedLong();
                    if (ifdOffset != 0) {
                        GalleryLog.w("ExifParser", "Invalid link to next IFD: " + ifdOffset);
                    }
                }
            }
        }
            ...
            try {
                ...
                if (event instanceof IfdEvent) {
                    ...
                    if (((this.mNumOfTagInIfd * 12) + this.mIfdStartOffset) + 2 > this.mApp1End) {
                        GalleryLog.w("ExifParser", "Invalid size of IFD " + this.mIfdType);
                        return 5;
                    }
                ...
                GalleryLog.w("ExifParser", "Failed to skip to data at: " + entry.getKey() + " for " + event.getClass().getName() + ", the file may be broken.");
            }
        }
        return 5;
    }
    ...
    private ExifTag readTag() throws IOException, ExifInvalidFormatException {
        ...
        if (numOfComp > 2147483647L) {
            ...
            GalleryLog.w("ExifParser", String.format("Tag %04x: Invalid data type %d", new Object[]{Short.valueOf(tagId), Short.valueOf(dataFormat)}));
    ...
    protected void readFullTagValue(ExifTag tag) throws IOException {
        ...
        if (this.mCorrespondingEvent.size() > 0 && ((Integer) this.mCorrespondingEvent.firstEntry().getKey()).intValue() < this.mTiffStream.getReadByteCount() + size) {
            ...
            if (event instanceof ImageEvent) {
                GalleryLog.w("ExifParser", "Thumbnail overlaps value for tag: \n" + tag.toString());
                GalleryLog.w("ExifParser", "Invalid thumbnail offset: " + this.mCorrespondingEvent.pollFirstEntry().getKey());
            } else {
                if (event instanceof IfdEvent) {
                    GalleryLog.w("ExifParser", "Ifd " + ((IfdEvent) event).ifd + " overlaps value for tag: \n" + tag.toString());
                } else if (event instanceof ExifTagEvent) {
                    GalleryLog.w("ExifParser", "Tag value for tag: \n" + ((ExifTagEvent) event).tag.toString() + " overlaps value for tag: \n" + tag.toString());
                }
                size = ((Integer) this.mCorrespondingEvent.firstEntry().getKey()).intValue() - this.mTiffStream.getReadByteCount();
                GalleryLog.w("ExifParser", "Invalid size of tag: \n" + tag.toString() + " setting count to: " + size);
    ...
    private boolean seekTiffData(InputStream inputStream) throws IOException, ExifInvalidFormatException {
        ...
        while (marker != (short) -39 && !JpegHeader.isSofMarker(marker)) {
            ...
            if (length < 2 || ((long) (length - 2)) != dataStream.skip((long) (length - 2))) {
                GalleryLog.w("ExifParser", "Invalid JPEG format.");
                return false;
            }
    ...
    protected String readString(int n) throws IOException {
        return readString(n, PARSER_CHARSET);
    }

...
class qr implements qo {
    private static final String Kf = TMSDKContext.getStrFromEnvMap(TMSDKContext.CON_HOST_URL);
    ...
    private volatile long Kt = 0;
    private volatile long Ku = 0;
    ...
    private void a(ec ecVar) {
        ...
        intent.putExtra(MapTilsCacheAndResManager.AUTONAVI_DATA_PATH, ecVar);
    ...
    private void a(fj fjVar, int i, String str, String str2, HashMap<String, Object> hashMap) {
        ...
        fjVar.aa(str);
        fjVar.ab(str2);
        fjVar.Z(XmlUtils.INPUT_ENCODING);
    ...
    private boolean hY() {
        ...
        this.mImei = this.yq.getString("imei", dk);
    ...
    int a(List<qs> list, boolean z, String str) {
        ...
        int i4 = 0;
        for (byte[] encrypt2 : arrayList) {
            System.arraycopy(encrypt2, 0, bArr, i4, encrypt2.length);
            i4 = encrypt2.length + i4;
        }
        mu muVar = null;
        try {
            encrypt2 = TccCryptor.encrypt(bArr, null);
            cA = mu.cA(str);
            try {
                cA.setRequestMethod("POST");
                cA.setPostData(encrypt2);
                cA.fc();
                AtomicReference atomicReference = new AtomicReference();
                i2 = cA.a(false, atomicReference);
                if (i2 == 0) {
                    List<fj> arrayList2 = new ArrayList(size + 1);
                    encrypt2 = (byte[]) atomicReference.get();
                    if (encrypt2 != null) {
                        if (encrypt2.length > 0) {
                            Object decrypt = TccCryptor.decrypt(encrypt2, null);
                            ...
                            while (i + 4 < length) {
                                ...
                                fjVar3.Z(XmlUtils.INPUT_ENCODING);
                                fjVar3.b(obj);
                    ...
                    if (bArr.length > 0) {
                        ...
                        if (i3 == decrypt.length) {
                            ...
                            if (i3 + i4 == decrypt.length) {
                                ...
                                fjVar4.Z(XmlUtils.INPUT_ENCODING);
                                fjVar4.b(obj2);
                                fjVar2.b(obj);
                                if (fjVar4.q() == q) {
                                    fj fjVar5 = fjVar2;
    ...
    synchronized int hZ() {
        ...
        this.yq.a("imei", this.mImei, false);

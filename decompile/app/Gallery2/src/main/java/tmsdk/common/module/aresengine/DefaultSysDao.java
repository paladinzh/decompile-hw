...
public final class DefaultSysDao extends AbsSysDao {
    ...
    private static final Uri Cj = Uri.parse("content://icc/adn");
    private c Ck;
    private jz Cl;
    private Context mContext;
    private lo uU = TMServiceFactory.getSysDBService();
    ...
    final class a implements c {
        private final Uri Cm = People.CONTENT_URI;
        private final Uri Cn = Phones.CONTENT_URI;
        ...
        public List<ContactEntity> fj() {
            ...
            Cursor query = this.Co.uU.query(this.Cm, new String[]{"_id", WMEditor.TYPENUM, "display_name"}, null, null, "name asc");
            if (this.Co.d(query)) {
    ...
    private CallLogEntity c(Cursor cursor) {
        ...
        callLogEntity.phonenum = cursor.getString(cursor.getColumnIndex(WMEditor.TYPENUM)).replaceAll("[ -]+", "");
        ...
        synchronized (Calls.CONTENT_URI) {
            try {
                ...
                try {
                    if (d(query)) {
                        while (!query.isAfterLast()) {
                            if (nf.cJ(query.getString(query.getColumnIndex(WMEditor.TYPENUM)))) {
        ...
        synchronized (Sms.CONTENT_URI) {
            try {
                ...
                if (d(query)) {
                    ...
                    if (i >= 0) {
                        if ((currentTimeMillis > ((long) (i * 1000)) ? 1 : null) == null) {

...
public class HanziToPinyin {
    ...
    private HanziToPinyin() {
        try {
            ...
            HwLog.w("HanziToPinyin", "Han-Latin/Names transliterator data is missing, HanziToPinyin is disabled");
        }
    }
        ...
        for (int i = 0; i < inputLength; i++) {
            ...
            if (!Character.isSpaceChar(character) && character != '.') {
                ...
                if (token.type == 2) {
                    ...
                    if (tokens.size() > 0) {
                        nameSb.append(HwCustPreloadContacts.EMPTY_STRING);
                    }
    ...
    public String getNormalizedStr(String input) {
        if (!hasChineseTransliterator() || TextUtils.isEmpty(input)) {
            return input;
        }
        int inputLength = input.length();

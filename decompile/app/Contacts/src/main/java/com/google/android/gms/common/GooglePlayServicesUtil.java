...
public final class GooglePlayServicesUtil extends zze {
    ...
    private static void zza(int i, Context context, String str) {
        ...
        if (zzmu.zzaw(context)) {
            zzx.zzab(zzne.zzsh());
            build = new Notification.Builder(context).setSmallIcon(R.drawable.common_ic_googleplayservices).setPriority(2).setAutoCancel(true).setStyle(new BigTextStyle().bigText(zzg + HwCustPreloadContacts.EMPTY_STRING + zzc)).addAction(R.drawable.common_full_open_on_phone, resources.getString(R.string.common_open_on_phone), zza).build();
        } else {

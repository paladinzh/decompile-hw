...
public class NormalMapLoader extends BaseMapLoader {
    ...
    public String getGridParmaV4() {
        ...
        if (str3.length() > 0) {
            str = str3;
            while (str != null) {
                if (!str.endsWith(str2) && !str.endsWith(HwCustPreloadContacts.EMPTY_STRING)) {
                    break;
                }
        ...
        if (i4 + b <= bArr.length && i4 <= bArr.length - 1 && b >= (byte) 0) {
            ...
            if (this.mGLMapEngine.isMapEngineValid()) {
                ...
                if (i2 > i) {
                    ...
                    f putData = instance.putData(obj);
                    if (this.mMapCallback.isGridInScreen(this.datasource, str)) {
                        i5 = 0;
                    } else {
                        i5 = 1;
                    ...
                    if (putData != null) {
                        this.mGLMapEngine.putMapData(putData.a, 0, putData.a.length, this.datasource, putData.c);
                    }
                    if (i5 != 0) {
                        doCancel();
                    }

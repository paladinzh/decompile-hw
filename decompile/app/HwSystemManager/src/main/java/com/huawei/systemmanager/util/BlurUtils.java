...
public class BlurUtils {
    ...
    public static Bitmap blurImage(Context ctx, Bitmap input, Bitmap output, int radius) {
        if (ctx == null || input == null || output == null || radius <= 0 || radius > 25) {
            Log.w(TAG, "blurImage() parameter is incorrect:" + ctx + ConstValues.SEPARATOR_KEYWORDS_EN + input + ConstValues.SEPARATOR_KEYWORDS_EN + output + ConstValues.SEPARATOR_KEYWORDS_EN + radius);
            return null;
        }
    ...
    public static Bitmap screenShotBitmap(Context ctx, int minLayer, int maxLayer, float scale) {
        Bitmap bitmap;
        ...
        if (isLazyMode(ctx)) {
            bitmap = SurfaceControl.screenshot(sourceCrop, dims[0], dims[1], 0, -1, false, 0);
        } else {

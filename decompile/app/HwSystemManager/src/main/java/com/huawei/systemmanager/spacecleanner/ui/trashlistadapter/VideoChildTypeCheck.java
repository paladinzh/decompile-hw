...
public class VideoChildTypeCheck extends ChildType {
    ...
    private static final SimpleImageLoadingListener sLoadListener = new SimpleImageLoadingListener() {
        public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
            if (view == null) {
                HwLog.e(VideoChildTypeCheck.TAG, "onLoadingComplete,view is null!");
                return;
            }
            if (view instanceof FrameBorderImageView) {
                FrameBorderImageView iconView = (FrameBorderImageView) view;
                iconView.setHasBorder(true);
                iconView.setScaleType(ScaleType.CENTER_CROP);
                iconView.setImageBitmap(loadedImage);
            } else {
                HwLog.e(VideoChildTypeCheck.TAG, "imageview must be FrameBorderImageView.");
            }
        }

        public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
            if (view == null) {
                HwLog.e(VideoChildTypeCheck.TAG, "onLoadingFailed,view is null!");
                return;
            }
            ITrashItem item = (ITrashItem) view.getTag();
            if (item == null) {
                HwLog.e(VideoChildTypeCheck.TAG, "onLoadingFailed,ITrashItem is null!");
                return;
            }
            if (view instanceof FrameBorderImageView) {
                FrameBorderImageView iconView = (FrameBorderImageView) view;
                if (item.getItemIcon() != null) {
                    iconView.setHasBorder(false);
                    iconView.setImageDrawable(item.getItemIcon());
                } else {
                    HwLog.e(VideoChildTypeCheck.TAG, "item's default icon is null.it is wrong!");
                }
            } else {
                HwLog.e(VideoChildTypeCheck.TAG, "imageview must be FrameBorderImageView.");
            }
    ...
    private static class ViewHolder extends ChildViewHolder {
        ...
        private final FrameBorderImageView trashIcon;
        ...
        public ViewHolder(ChildType type, View convertView) {
            ...
            this.trashIcon = (FrameBorderImageView) convertView.findViewById(R.id.image);
        }
    }

    public VideoChildTypeCheck(LayoutInflater inflater, OnClickListener checkClicker, OnClickListener itemClicker, OnLongClickListener longClicker) {
        super(inflater);
    ...
    public View newView(int groupPosition, int childPosition, boolean isLastChild, ViewGroup parent, ITrashItem item) {
        ...
        if (this.options == null) {
            this.options = new Builder().cacheInMemory(true).cacheOnDisk(false).showImageOnLoading(item.getIcon(GlobalContext.getContext())).considerExifParams(true).build();
        }
        return convertView;
    }
    ...
    public void bindView(boolean isLastChild, View convertView, ITrashItem item) {
        ...
        holder.trashIcon.setHasBorder(true);
        holder.trashIcon.setTag(item);
        if (this.options != null) {
            ImageLoader.getInstance().displayImage(Utility.getLocalPath(item.getTrashPath()), holder.trashIcon, this.options, sLoadListener);
        }

...
public class ViewDragHelper {
    ...
    private static final Interpolator sInterpolator = new Interpolator() {
        public float getInterpolation(float t) {
            t -= Utility.ALPHA_MAX;
            return ((((t * t) * t) * t) * t) + Utility.ALPHA_MAX;
        }
    };
...
/*
Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.huawei.systemmanager.comm.widget.slideview.ViewDragHelper.clearMotionHistory(int):void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.huawei.systemmanager.comm.widget.slideview.ViewDragHelper.clearMotionHistory(int):void");
    }
    ...
    public static ViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        ...
        helper.mTouchSlop = (int) (((float) helper.mTouchSlop) * (Utility.ALPHA_MAX / sensitivity));
        return helper;
    }
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        if (this.mCustomDuration >= 0) {
            return computeCustomAxisDuration(delta, velocity, motionRange);
        }
        int duration;
        int width = this.mParentView.getWidth();
        int halfWidth = width / 2;
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(Utility.ALPHA_MAX, ((float) Math.abs(delta)) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(Math.abs(distance / ((float) velocity)) * 1000.0f) * 4;
        } else {
            duration = (int) (((((float) Math.abs(delta)) / ((float) motionRange)) + Utility.ALPHA_MAX) * 256.0f);
        }
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case 3:
                ...
                break;
            case 2:
                doInterceptActionMove(ev);
                break;
            case 5:
                donInterceptActionPointerDown(ev, actionIndex);
                break;
            case 6:
    ...
    private void donInterceptActionDown(MotionEvent ev) {
        float x = ev.getX();
        float y = ev.getY();
    ...
    private void donInterceptActionPointerDown(MotionEvent ev, int actionIndex) {
    ...
    private void doInterceptActionMove(MotionEvent ev) {
        int pointerCount = MotionEventCompat.getPointerCount(ev);
        for (int i = 0; i < pointerCount && this.mInitialMotionX != null && this.mInitialMotionY != null; i++) {
            ...
            if (pointerId < this.mInitialMotionX.length && pointerId < this.mInitialMotionY.length) {
                ...
                if (this.mDragState != 1) {
                    View toCapture = findTopChildUnder((int) this.mInitialMotionX[pointerId], (int) this.mInitialMotionY[pointerId]);
    ...
    private int computeCustomAxisDuration(int delta, int velocity, int motionRange) {
        return Math.max((Math.abs(delta) * this.mCustomDuration) / motionRange, 256);
    }

    void setInterpolator(Context context, Interpolator interpolator) {
        this.mScroller = ScrollerCompat.create(context, interpolator);
        if (interpolator instanceof DurationInterpolator) {
            this.mCustomDuration = ((DurationInterpolator) interpolator).getDuration();
        }
    }
}

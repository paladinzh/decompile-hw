...
public class HwCustManager {
    ...
    private static volatile HwCustManager sInstance = null;
    ...
    public <T> T getHwCustObj(Class<T> classClass, Object... args) {
        return getHwCustObj(classClass, false, args);
    }

    public <T> T getHwCustObj(Class<T> classClass, boolean isClear, Object... args) {
        if (this.mCreateObjMethod == null) {
            return null;
        }
        T obj = this.mCustHashMap.get(classClass);
        if (isClear) {

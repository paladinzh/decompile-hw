...
public class OnePkgMultiItemQuery extends AbsRecommendQuery {
    ...
    protected List<Integer> getItemIdList(Bundle bundle) {
        try {
            return RecommendQueryInput.extractItemIdListFromBundle(2, bundle);
    ...
    protected List<String> getRangeOfPackages(Bundle bundle) {
        try {
            return Lists.newArrayList(RecommendQueryInput.extractPkgNameFromBundle(2, bundle));
        } catch (RecommendParamException ex) {

...
final class FragmentManagerImpl extends FragmentManager implements LayoutInflaterFactory {
    ...
    Animation loadAnimation(Fragment fragment, int transit, boolean enter, int transitionStyle) {
        ...
        switch (styleIndex) {
            case 1:
                return makeOpenCloseAnimation(this.mHost.getContext(), 1.125f, Utility.ALPHA_MAX, 0.0f, Utility.ALPHA_MAX);
            case 2:
                return makeOpenCloseAnimation(this.mHost.getContext(), Utility.ALPHA_MAX, 0.975f, Utility.ALPHA_MAX, 0.0f);
            case 3:
                return makeOpenCloseAnimation(this.mHost.getContext(), 0.975f, Utility.ALPHA_MAX, 0.0f, Utility.ALPHA_MAX);
            case 4:
                return makeOpenCloseAnimation(this.mHost.getContext(), Utility.ALPHA_MAX, 1.075f, Utility.ALPHA_MAX, 0.0f);
            case 5:
                return makeFadeAnimation(this.mHost.getContext(), 0.0f, Utility.ALPHA_MAX);
            case 6:
                return makeFadeAnimation(this.mHost.getContext(), Utility.ALPHA_MAX, 0.0f);
            default:
    ...
    void moveToState(Fragment f, int newState, int transit, int transitionStyle, boolean keepActive) {
        ...
        throwException(new IllegalArgumentException("No view found for id 0x" + Integer.toHexString(f.mContainerId) + SqlMarker.LEFT_PARENTHESES + resName + ") for fragment " + f));
        f.mContainer = r11;
        f.mView = f.performCreateView(f.getLayoutInflater(f.mSavedFragmentState), r11, f.mSavedFragmentState);
        if (f.mView != null) {
            f.mInnerView = f.mView;
            if (VERSION.SDK_INT >= 11) {
                ViewCompat.setSaveFromParentEnabled(f.mView, false);

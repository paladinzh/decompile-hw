...
public class SlidingPaneLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode != 1073741824) {
            if (!isInEditMode()) {
                ...
                if (widthMode == 0) {
                    widthSize = IUpdateListener.ERROR_CODE_NO_NETWORK;
                }
            }
            ...
            if (!isInEditMode()) {
                ...
                heightSize = IUpdateListener.ERROR_CODE_NO_NETWORK;
            }
        }
        int layoutHeight = 0;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mFirstLayout) {
            float f = (this.mCanSlide && this.mPreservedOpenState) ? Utility.ALPHA_MAX : 0.0f;
            this.mSlideOffset = f;
        }
        for (i = 0; i < childCount; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != 8) {
                ...
                if (lp.slideable) {
                    ...
                    offset = (int) ((Utility.ALPHA_MAX - this.mSlideOffset) * ((float) this.mParallaxBy));
                    xStart = nextXStart;
                }
                if (isLayoutRtl) {
    ...
    private boolean openPane(View pane, int initialVelocity) {
        if (!this.mFirstLayout && !smoothSlideTo(Utility.ALPHA_MAX, initialVelocity)) {
            return false;
        }
    ...
    public boolean isOpen() {
        return !this.mCanSlide || this.mSlideOffset == Utility.ALPHA_MAX;
    }
    ...
    private void parallaxOtherViews(float slideOffset) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (v != this.mSlideableView) {
                int oldOffset = (int) ((Utility.ALPHA_MAX - this.mParallaxOffset) * ((float) this.mParallaxBy));
                this.mParallaxOffset = slideOffset;
                int dx = oldOffset - ((int) ((Utility.ALPHA_MAX - slideOffset) * ((float) this.mParallaxBy)));
                if (isLayoutRtl) {
                    dx = -dx;
                }
                ...
                if (dimViews) {
                    float f;
                    if (isLayoutRtl) {
                        f = this.mParallaxOffset - Utility.ALPHA_MAX;
                    } else {
                        f = Utility.ALPHA_MAX - this.mParallaxOffset;
                    }

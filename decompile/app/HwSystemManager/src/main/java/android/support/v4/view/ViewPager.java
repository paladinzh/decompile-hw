...
public class ViewPager extends ViewGroup {
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(Utility.ALPHA_MAX, (((float) Math.abs(dx)) * Utility.ALPHA_MAX) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(Math.abs(distance / ((float) velocity)) * 1000.0f) * 4;
        } else {
            duration = (int) ((Utility.ALPHA_MAX + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + -1 ? (curItem.offset + curItem.widthFactor) - Utility.ALPHA_MAX : AutoScrollHelper.NO_MAX;
        int i = curIndex - 1;
        while (i >= 0) {
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N - 1) {
                this.mLastOffset = (ii.widthFactor + offset) - Utility.ALPHA_MAX;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i++;
        ...
        for (i = 0; i < size; i++) {
            ...
            if (child.getVisibility() != 8) {
                ...
                if (lp != null && lp.isDecor) {
                    ...
                    int vgrav = lp.gravity & Events.E_ADDVIEW_SET_ALL;
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == 1 && this.mAdapter != null && this.mAdapter.getCount() > 1)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                width = getWidth();
                height = (getHeight() - getPaddingTop()) - getPaddingBottom();
                canvas.rotate(CircleView.START_ANGLE);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + Utility.ALPHA_MAX)) * ((float) width));
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        switch (event.getKeyCode()) {
            ...
            case Events.E_ANTISPAM_CLEAR_MSG /*61*/:
                if (VERSION.SDK_INT < 11) {
                    return false;
                }

...
public final class Locale implements Cloneable, Serializable {
    ...
    static {
        boolean z;
        if (Locale.class.desiredAssertionStatus()) {
            z = false;
        } else {
            z = true;
        }
    ...
    public final String toString() {
        ...
        boolean e = (this.localeExtensions == null || this.localeExtensions.getID().length() == 0) ? false : true;
    ...
    private static boolean isValidVariantSubtag(String subTag) {
        if (subTag.length() < 5 || subTag.length() > 8) {
            if (subTag.length() == 4) {
                ...
                return firstChar >= '0' && firstChar <= '9' && isAsciiAlphaNum(subTag);
            }

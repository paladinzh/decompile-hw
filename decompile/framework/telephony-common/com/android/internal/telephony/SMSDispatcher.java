...
public abstract class SMSDispatcher extends AbstractSMSDispatcher {
    ...
    private void handleSendSmsOverLoad() {
        if (this.mAlreadySentSms.get() > 50) {
            ...
            for (int i = 0; i < this.mPackageSendSmsCount.size(); i++) {
    ...
    protected void handleSendComplete(AsyncResult ar) {
        ...
        if (ar.exception == null) {
            ...
            if (tracker.mImsRetry > 0 && ss != 0) {
                tracker.mRetryCount = isViaAndCdma() ? 3 : 1;
    ...
    protected void handleReachSentLimit(SmsTracker tracker) {
        if (!denyIfQueueLimitReached(tracker)) {
            ...
            Spanned messageText = Html.fromHtml(r.getString(17040352, new Object[]{appLabel}));
    ...
    protected void handleConfirmShortCode(boolean isPremium, SmsTracker tracker) {
        if (!denyIfQueueLimitReached(tracker)) {
            ...
            Resources r = Resources.getSystem();
            Spanned messageText = Html.fromHtml(r.getString(17040355, new Object[]{appLabel, tracker.mDestAddress}));
            View layout = ((LayoutInflater) new ContextThemeWrapper(this.mContext, r.getIdentifier("androidhwext:style/Theme.Emui", null, null)).getSystemService("layout_inflater")).inflate(17367271, null);
    ...
    protected SmsTracker getSmsTracker(HashMap<String, Object> data, PendingIntent sentIntent, PendingIntent deliveryIntent, String format, AtomicInteger unsentPartCount, AtomicBoolean anyPartFailed, Uri messageUri, SmsHeader smsHeader, boolean isExpectMore, String fullMessageText, boolean isText, boolean persistMessage) {
        ...
        return new SmsTracker(data, sentIntent, deliveryIntent, packageInfo, PhoneNumberUtils.extractNetworkPortion((String) data.get("destAddr")), format, unsentPartCount, anyPartFailed, messageUri, smsHeader, isExpectMore, fullMessageText, getSubId(), isText, persistMessage);
    }

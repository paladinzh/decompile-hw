...
public class ImsPhone extends ImsPhoneBase {
    ...
    Phone mDefaultPhone;
    private boolean mDelayedRoaming;
        ...
        if (!this.mHwImsPhone.beforeHandleMessage(msg)) {
            switch (msg.what) {
                ...
                case 51:
                    ...
                    break;
                case 52:
                    if (ar.result instanceof ServiceState) {
                        boolean isRoaming = ar.result.getRoaming();
                        if (this.mRoaming != isRoaming) {
                            Rlog.d(LOG_TAG, "Roaming state changed");
                            onRoamingStateChange(isRoaming);
                            break;
                        }
                    }
                    break;
                case 53:
                    if (this.mDelayedRoaming) {
                        Rlog.d(LOG_TAG, "Voice call ended. Handle pending onRoamingStateChange.");
                        this.mDelayedRoaming = false;
                        boolean newRoaming = getCurrentRoaming();
                        if (this.mRoaming != newRoaming) {
                            Rlog.d(LOG_TAG, "Roaming state changed");
                            onRoamingStateChange(newRoaming);
                            break;
                        }
                    }
                    break;
    ...
    private void onRoamingStateChange(boolean newRoaming) {
        if (this.mCT.getState() == PhoneConstants.State.IDLE) {
            Rlog.d(LOG_TAG, "onRoamingStateChange now: " + newRoaming);
            this.mRoaming = newRoaming;
            ImsManager.setWfcMode(this.mContext, ImsManager.getWfcMode(this.mContext, newRoaming), newRoaming);
            return;
        }
        Rlog.d(LOG_TAG, "onRoamingStateChange delayed: " + newRoaming);
        this.mDelayedRoaming = true;
    }
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        pw.println("  mDelayedRoaming = " + this.mDelayedRoaming);

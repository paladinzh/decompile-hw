...
public class ImsPhoneCallTracker extends AbstractImsPhoneCallTracker implements ImsPullCall {
    ...
    public boolean canDial() {
        ...
        if (!(fgCallState == State.IDLE || fgCallState == State.DISCONNECTED)) {
            if (fgCallState == State.ACTIVE) {
            }
            return z2;
        }
        if (!(bgCallState == State.IDLE || bgCallState == State.DISCONNECTED || bgCallState == State.HOLDING)) {
            z = false;
        }
        z2 = z;
        return z2;

...
public class WifiServiceImpl extends Stub {
    ...
    public void startScan(ScanSettings settings, WorkSource workSource) {
        ...
        if (this.mWifiStateMachine.allowWifiScanRequest(Binder.getCallingPid())) {
            Slog.d(TAG, "wifi_scan reject because the interval isn't arrived");
            return;
        }
    ...
    private void showP2pToStaDialog() {
        ...
        AlertDialog dialog = new Builder(this.mContext, 33947691).setCancelable(false).setTitle(r.getString(33685804)).setMessage(r.getString(33685805)).setView(checkBox).setNegativeButton(r.getString(17039360), new OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                WifiServiceImpl.this.setWifiEnabled(true);
            }
        }).create();
        dialog.getWindow().setType(2014);
        dialog.show();
    ...
    private void logScanResultsListRestrictively(String callingPackage, List<ScanResult> scanResultsList) {
        long currentLogTime = SystemClock.elapsedRealtime();

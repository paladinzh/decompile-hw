...
public final class AssetManager implements AutoCloseable {
    ...
    private static boolean resValid = false;
    ...
    private static String getParentLocale(String locale) {
        ...
        parentlist[56] = new String[]{"fil_PH", "tl_PH"};
        parentlist[57] = new String[]{"fil", "tl_PH"};
    ...
    final CharSequence getResourceBagText(int resId, int bagEntryId) {
        synchronized (this) {
            TypedValue outValue = this.mValue;
            int block = loadResourceBagValue(hasRes() ? resId | Integer.MIN_VALUE : resId, bagEntryId, outValue, true);
            if (block < 0) {
                return null;
    ...
    final String[] getResourceStringArray(int resId) {
        int dbid_id;
        if (hasRes()) {
            dbid_id = resId | Integer.MIN_VALUE;
        } else {
            dbid_id = resId;
        }
        ...
        if (outValue.type == 3) {
            ...
            synchronized (this) {
                if (!inited || hasSharedRes) {
                    TypedValue tmpValue = this.mDbidValue;
                    int dbidblock = loadResourceValue(resId | Integer.MIN_VALUE, (short) densityDpi, tmpValue, resolveRefs);
                    if (dbidblock >= 0 && tmpValue.type == 3) {

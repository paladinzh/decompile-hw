...
public class ZenModeConfig implements Parcelable {
    ...
    public static Condition toTimeCondition(Context context, long time, int minutes, int userHandle, boolean shortVersion) {
        ...
        if (minutes < 60) {
            ...
            line2 = res.getString(17040804, formattedTime);
    ...
    public static Condition toNextAlarmCondition(Context context, long now, long alarm, int userHandle) {
        ...
        return new Condition(toCountdownConditionId(alarm), ProxyInfo.LOCAL_EXCL_LIST, context.getResources().getString(17040805, formattedTime), ProxyInfo.LOCAL_EXCL_LIST, 0, 1, 1);
    }
    ...
    public static Uri toEventConditionId(EventInfo event) {
        return new Builder().scheme("condition").authority(SYSTEM_AUTHORITY).appendPath("event").appendQueryParameter("userId", Long.toString((long) event.userId)).appendQueryParameter("calendar", event.calendar != null ? event.calendar : ProxyInfo.LOCAL_EXCL_LIST).appendQueryParameter("reply", Integer.toString(event.reply)).build();
    }
        ...
        for (ZenRule automaticRule : config.automaticRules.values()) {
            if (automaticRule.isAutomaticActive()) {
                if (summary.isEmpty()) {
                    ...
                    summary = context.getResources().getString(17040808, summary, automaticRule.name);
                }
            }
        }
        return summary;

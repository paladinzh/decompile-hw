...
public class GLU {
    ...
    public static void gluOrtho2D(GL10 gl, float left, float right, float bottom, float top) {
        gl.glOrthof(left, right, bottom, top, ScaledLayoutParams.SCALE_UNSPECIFIED, 1.0f);
    }
    ...
    public static int gluProject(float objX, float objY, float objZ, float[] model, int modelOffset, float[] project, int projectOffset, int[] view, int viewOffset, float[] win, int winOffset) {
        ...
        synchronized (scratch) {
            ...
            float rw = 1.0f / w;
            win[winOffset] = ((float) view[viewOffset]) + ((((float) view[viewOffset + 2]) * ((scratch[20] * rw) + 1.0f)) * NetworkHistoryUtils.RECOVERY_PERCENTAGE);
            win[winOffset + 1] = ((float) view[viewOffset + 1]) + ((((float) view[viewOffset + 3]) * ((scratch[21] * rw) + 1.0f)) * NetworkHistoryUtils.RECOVERY_PERCENTAGE);
            win[winOffset + 2] = ((scratch[22] * rw) + 1.0f) * NetworkHistoryUtils.RECOVERY_PERCENTAGE;
            return 1;
        }
    }

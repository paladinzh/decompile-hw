...
public abstract class BatteryStats implements Parcelable {
    ...
    static final String[] SCREEN_BRIGHTNESS_SHORT_NAMES = new String[]{WifiEnterpriseConfig.ENGINE_DISABLE, WifiEnterpriseConfig.ENGINE_ENABLE, "2", "3", "4"};
    ...
    static {
        ...
        int i = 3;
        r7[15] = new BitDescription(56, i, "phone_signal_strength", "Pss", SignalStrength.SIGNAL_STRENGTH_NAMES, new String[]{WifiEnterpriseConfig.ENGINE_DISABLE, WifiEnterpriseConfig.ENGINE_ENABLE, "2", "3", "4"});
        ...
        r7[8] = new BitDescription(4194304, "bluetooth", "b");
        i = 4;
        r7[9] = new BitDescription(112, i, "wifi_signal_strength", "Wss", new String[]{WifiEnterpriseConfig.ENGINE_DISABLE, WifiEnterpriseConfig.ENGINE_ENABLE, "2", "3", "4"}, new String[]{WifiEnterpriseConfig.ENGINE_DISABLE, WifiEnterpriseConfig.ENGINE_ENABLE, "2", "3", "4"});
        ...
        if (reqUid < 0) {
            ...
            if (kernelWakelocks.size() > 0) {
                for (Entry<String, ? extends Timer> ent : kernelWakelocks.entrySet()) {
                    ...
                    dumpLine(pw, 0, category, KERNEL_WAKELOCK_DATA, ent.getKey(), stringBuilder.toString());
                }
            }
                ...
                if (mobileBytesRx <= 0 && mobileBytesTx <= 0 && wifiBytesRx <= 0 && wifiBytesTx <= 0 && mobilePacketsRx <= 0 && mobilePacketsTx <= 0 && wifiPacketsRx <= 0 && wifiPacketsTx <= 0 && mobileActiveTime <= 0 && mobileActiveCount <= 0 && btBytesRx <= 0) {
                    ...
                    if (fullWifiLockOnTime == 0 && wifiScanTime == 0 && wifiScanCount == 0) {
                        ...
                        for (iw = wakelocks.size() - 1; iw >= 0; iw--) {
                            ...
                            stringBuilder.setLength(0);
                            linePrefix = printWakeLockCheckin(stringBuilder, wl.getWakeTime(2), rawRealtime, "w", which, printWakeLockCheckin(stringBuilder, wl.getWakeTime(0), rawRealtime, TtmlUtils.TAG_P, which, printWakeLockCheckin(stringBuilder, wl.getWakeTime(1), rawRealtime, FullBackup.FILES_TREE_TOKEN, which, linePrefix)));
                    ...
                    dumpLine(pw, uid, category, WIFI_DATA, Long.valueOf(fullWifiLockOnTime), Long.valueOf(wifiScanTime), Long.valueOf(uidWifiRunningTime), Integer.valueOf(wifiScanCount), Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(0));
                    dumpControllerActivityLine(pw, uid, category, WIFI_CONTROLLER_DATA, u.getWifiControllerActivity(), which);
                    dumpTimer(pw, uid, category, BLUETOOTH_MISC_DATA, u.getBluetoothScanTimer(), rawRealtime, which);
                    dumpControllerActivityLine(pw, uid, category, BLUETOOTH_CONTROLLER_DATA, u.getBluetoothControllerActivity(), which);
                    if (u.hasUserActivity()) {
                        ...
                        if (fullWifiLockOnTime == 0 && wifiScanTime == 0 && wifiScanCount == 0) {
                            ...
                            if (wakelocks != null) {
                                for (iw = wakelocks.size() - 1; iw >= 0; iw--) {
                                    ...
                                    str = printWakeLock(sb, wl.getWakeTime(18), rawRealtime, "draw", which, printWakeLock(sb, wl.getWakeTime(2), rawRealtime, "window", which, printWakeLock(sb, wl.getWakeTime(0), rawRealtime, "partial", which, printWakeLock(sb, wl.getWakeTime(1), rawRealtime, "full", which, ": "))));
                                    sb.append(" realtime");
                                    pw.println(sb.toString());
                                    printTimer = true;
                                    countWakelock++;

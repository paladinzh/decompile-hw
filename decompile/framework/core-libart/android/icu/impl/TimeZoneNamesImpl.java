...
public class TimeZoneNamesImpl extends TimeZoneNames {
    ...
    private static class NameSearchHandler implements ResultHandler<NameInfo> {
        static final /* synthetic */ boolean -assertionsDisabled = (!NameSearchHandler.class.desiredAssertionStatus());
            ...
            for (Entry<Key, ZNamesLoader> entry : this.keyToLoader.entrySet()) {
                ...
                if (loader != ZNamesLoader.DUMMY_LOADER) {
                    if (key.startsWith(TimeZoneNamesImpl.MZ_PREFIX)) {
                        TimeZoneNamesImpl.this._mzNamesMap.put(mzIDFromKey(key).intern(), ZNames.getInstance(loader.getNames(), null));
                    } else {
                        String tzID = tzIDFromKey(key).intern();
                        TimeZoneNamesImpl.this._tzNamesMap.put(tzID, ZNames.getInstance(loader.getNames(), tzID));
                    }
                }
            }
        }

...
public class TimeZoneGenericNames implements Serializable, Freezable<TimeZoneGenericNames> {
    ...
    static {
        boolean z;
        if (TimeZoneGenericNames.class.desiredAssertionStatus()) {
            z = false;
        } else {
            z = true;
        }
    ...
    private synchronized String formatPattern(Pattern pat, String... args) {
        ...
        if (this._patternFormatters[idx] == null) {
            String patText;
            try {
                patText = ((ICUResourceBundle) UResourceBundle.getBundleInstance("android/icu/impl/data/icudt56b/zone", this._locale)).getStringWithFallback("zoneStrings/" + pat.key());

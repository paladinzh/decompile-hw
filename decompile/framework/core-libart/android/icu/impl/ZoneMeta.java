...
public final class ZoneMeta {
    ...
    private static class CustomTimeZoneCache extends SoftCache<Integer, SimpleTimeZone, int[]> {
        static final /* synthetic */ boolean -assertionsDisabled = (!CustomTimeZoneCache.class.desiredAssertionStatus());
    ...
    static {
        boolean z;
        if (ZoneMeta.class.desiredAssertionStatus()) {
            z = false;
        } else {
            z = true;
        }
    ...
    private static String getShortIDFromCanonical(String canonicalID) {
        String shortID = null;
        try {
            shortID = UResourceBundle.getBundleInstance("android/icu/impl/data/icudt56b", "keyTypeData", ICUResourceBundle.ICU_DATA_CLASS_LOADER).get("typeMap").get("timezone").getString(canonicalID.replace('/', ':'));
        } catch (MissingResourceException e) {
        }
        return shortID;
    }

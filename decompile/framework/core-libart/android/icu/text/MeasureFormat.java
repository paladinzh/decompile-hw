...
public class MeasureFormat extends UFormat {
        ...
        class UnitPatternSink extends TableSink {
            ...
            void setFormatterIfAbsent(int index, Value value, int minPlaceholders) {
                if (this.patterns == null) {
                    EnumMap<FormatWidth, String[]> styleToPatterns = (EnumMap) UnitDataSink.this.cacheData.unitToStyleToPatterns.get(UnitDataSink.this.unit);
                    if (styleToPatterns == null) {
                        ...
                        this.patterns = (String[]) styleToPatterns.get(UnitDataSink.this.width);
                    }
                    if (this.patterns == null) {
        ...
        class UnitTypeSink extends TableSink {
            ...
            public TableSink getOrCreateTableSink(Key key, int initialSize) {
                if (!key.contentEquals("currency")) {
                    ...
                    } else if (!UnitDataSink.this.cacheData.hasPerFormatter(UnitDataSink.this.width)) {
    ...
    public final String formatMeasureRange(Measure lowValue, Measure highValue) {
        ...
        if (unit.equals(highValue.getUnit())) {
            ...
            String formattedNumber = SimplePatternFormatter.formatCompiledPattern(getRangeFormat(getLocale(), this.formatWidth), stringBuffer, highFormatted);
            if (isCurrency) {
            ...
            return SimplePatternFormatter.formatCompiledPattern(getPluralFormatter(lowValue.getUnit(), this.formatWidth, resolvedPlural.ordinal()), formattedNumber);
        }
    ...
    private static MeasureFormatData loadLocaleData(ULocale locale) {
        ...
        resource.getAllTableItemsWithFallback("", new UnitDataSink(cacheData));
        return cacheData;
    }

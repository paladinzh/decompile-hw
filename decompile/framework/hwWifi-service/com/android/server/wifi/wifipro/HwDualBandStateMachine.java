...
public class HwDualBandStateMachine extends StateMachine {
    ...
    private boolean mCHRIsManualHandover = false;
                ...
                if (info.mDualBandApInfoRcd != null) {
                    ...
                    if (info.mDualBandApInfoRcd.mDisappearCount > 3) {
                        ...
                        if (info.mIsDualbandAP == 1) {
                            WifiProStatisticsManager.getInstance().increaseDualbandStatisticCount(5);
    ...
    private int dualbandChrGetAPType() {
        ...
        HwDualBandMonitorInfo info = (HwDualBandMonitorInfo) this.mMonitorAPList.get(0);
    ...
    private void reportDualbandChrHandoverTooSlow() {
        if (this.mCHRIsManualHandover) {
            ...
            if (mWifiInfo == null || mWifiInfo.getBSSID() == null || mWifiInfo.getSSID() == null) {
                ...
                return;
            } else if (!ScanResult.is5GHz(mWifiInfo.getFrequency())) {
                return;
            } else {
                if (this.mCHRSavedAPList.size() != 0) {
                    for (HwDualBandMonitorInfo info : this.mCHRSavedAPList) {
                        if (info.mBssid.equals(mWifiInfo.getBSSID()) && this.mCHRHandoverTooSlow != null) {
                            ...
                            WifiProStatisticsManager.getInstance().uploadWifiProDualbandExceptionEvent("DUALBAND_HANDOVER_TOO_SLOW", this.mCHRHandoverTooSlow);
    ...
    private void reportDualbandChrMonitorCount() {
        if (dualbandChrGetAPType() == 1) {
            WifiProStatisticsManager.getInstance().increaseDualbandStatisticCount(3);
        } else {
    ...
    private void reportDualbandChrFreqScan5GCount() {
        if (dualbandChrGetAPType() == 1) {
            if (this.mCHRSingleAPScanCount <= 3) {
                WifiProStatisticsManager.getInstance().increaseDualbandStatisticCount(10);
            } else if (this.mCHRSingleAPScanCount <= 5) {
        ...
        } else if (this.mCHRSingleAPScanCount <= 3) {

...
public class HuaweiWifiWatchdogStateMachine extends StateMachine {
    ...
    private static final GoodSpdCountRate[] GOOD_SPD_COUNT_RATE_TBL = new GoodSpdCountRate[]{new GoodSpdCountRate(25.0f, WifiProHistoryRecordManager.HOME_AP_LEVEL_THREE_SWITCH_RATE), new GoodSpdCountRate(20.0f, 0.6f), new GoodSpdCountRate(15.0f, WifiProHistoryRecordManager.HOME_AP_LEVEL_ONE_SWITCH_RATE), new GoodSpdCountRate(10.0f, SCORE_BACK_OFF_RATE), new GoodSpdCountRate(5.0f, 0.9f)};
    ...
    private static final RssiRate[] RSSI_RATE_TBL = new RssiRate[]{new RssiRate(-50, 0.6f), new RssiRate(-65, WifiProHistoryRecordManager.HOME_AP_LEVEL_ONE_SWITCH_RATE), new RssiRate(-75, SCORE_BACK_OFF_RATE), new RssiRate(QOS_NOT_GOOD_LEVEL_OF_RSSI, 0.9f)};
    ...
    private int RSSI_TH_FOR_BAD_QOE_LEVEL = POOR_NET_MIN_RSSI_TH;
    private int RSSI_TH_FOR_NOT_BAD_QOE_LEVEL = MAY_BE_POOR_RSSI_TH;
    ...
    private float mHighDataFlowRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
    ...
    class WifiProLinkMonitoringState extends State {
        ...
        private float mGoodSpeedRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
        private float mHomeApSwichRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
        ...
        private float mRssiRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
        ...
        public void enter() {
            ...
            this.mRssiRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
            ssidChangeDetection();
            ...
            this.mHomeApSwichRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
        ...
        private boolean detectNetworkAvailable(int tcpTxPkts, int tcpRxPkts, int tcpRetransPkts, String currentBssid) {
            ...
            if (HuaweiWifiWatchdogStateMachine.this.mWifiProHistoryRecordManager != null && this.mNetworkDisableCount >= 2) {
                ...
                detectCountRequirement = this.mHomeApSwichRate <= 0.3f ? 5 : this.mHomeApSwichRate <= WifiProHistoryRecordManager.HOME_AP_LEVEL_THREE_SWITCH_RATE ? 4 : this.mHomeApSwichRate <= WifiProHistoryRecordManager.HOME_AP_LEVEL_ONE_SWITCH_RATE ? 3 : 2;
            }
            ...
            if (this.mNetworkDisableCount == 2) {
        ...
        private float getRssiSWRate(int rssi) {
            ...
            float retRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
            if (rssi < -75) {
                return WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
            }
            for (int i = 0; i < len; i++) {
        ...
        private void resetSpdGoodCounter() {
            this.mGoodSpeedRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
        ...
        private void updateSpdGoodCounter() {
            ...
            if (HuaweiWifiWatchdogStateMachine.this.mPeriodGoodSpdScore > 0.0f) {
                ...
                if (((float) HuaweiWifiWatchdogStateMachine.this.mSpeedNotGoodCount) >= HuaweiWifiWatchdogStateMachine.BACKOFF_NOT_SPD_GOOD_PERIOD_COUNT) {
                    huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
                    huaweiWifiWatchdogStateMachine.mSpeedGoodCount = huaweiWifiWatchdogStateMachine.mSpeedGoodCount - WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
        ...
        private float getCurrPeriodScore(int rssi, boolean rssiTcpBad, boolean otaBad, boolean tcpBad, boolean veryBadRTT, String currentBssid) {
            ...
            if (WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE != HuaweiWifiWatchdogStateMachine.this.mHighDataFlowRate) {
                retScore *= HuaweiWifiWatchdogStateMachine.this.mHighDataFlowRate;
            }
            if (!(WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE == this.mGoodSpeedRate || this.mPktChkBadCnt != 0 || WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE == homeApRate)) {
                retScore *= this.mGoodSpeedRate;
            }
            this.noHomeAPSwitchScore += retScore;
            if (WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE != homeApRate && rssi >= -75) {
                retScore *= homeApRate;
            }
        ...
        private void updateHighDataFlowRate() {
            ...
            if (HuaweiWifiWatchdogStateMachine.this.mHighDataFlowRate > WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE || HuaweiWifiWatchdogStateMachine.this.mHighDataFlowRate < 0.0f) {
                ...
                HuaweiWifiWatchdogStateMachine.this.mHighDataFlowRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
            }
        ...
        private void tryBackOffScore(long periodMaxSpeed, int avgRtt) {
            if (this.mSwitchScore > 0.0f) {
                ...
                if (isScoreBackoffSpeedGood || this.mSwitchScore < WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE) {
                    resetPoorNetState();
                    return;
                } else if (isScoreBackoffRTTGood) {
                    this.mSwitchScore *= WifiProHistoryRecordManager.HOME_AP_LEVEL_THREE_SWITCH_RATE;
                    this.noHomeAPSwitchScore *= WifiProHistoryRecordManager.HOME_AP_LEVEL_THREE_SWITCH_RATE;
                            ...
                            if (this.mPktChkCnt >= 1) {
                                ...
                                if (pTot > 0) {
                                    ...
                                    if (!this.isFirstEnterMontoringState) {
                                        ...
                                        if (processedTcpResult.mAvgRtt < HwWifiStateMachine.AP_CAP_CACHE_COUNT && processedTcpResult.mTotPkt > 2 && lossRate < HuaweiWifiWatchdogStateMachine.PKT_CHK_POOR_LINK_MIN_LOSE_RATE && pTot > 8 && tcpRetransRate < 0.1d && tcpTxPkts > 2) {
                                        ...
                                        boolean isRttBadForLowRssi = (processedTcpResult.mAvgRtt > 2000 || processedTcpResult.mAvgRtt == 0) ? processedTcpResult.mInadequateAvgRtt <= WifiproBqeUtils.BQE_GOOD_RTT ? processedTcpResult.mInadequateAvgRtt == 0 : true : false;
                                        ...
                                        if (isRssiBad) {
                                            ...
                                            if (isTCPBad || isOTABad || this.isLastTCPBad || this.isLastOTABad || tcpTxPkts <= 3 || tcpRxPkts <= 0) {
                                                this.adjustRssiCounter = 0;
                                            } else {
    ...
    class WifiProVerifyingLinkState extends State {
        ...
        private void tryRecoverWifiByOta(int current_rssi) {
            if (this.mIsReportWiFiGoodAllow) {
                ...
                boolean isRoTimerTimeout = this.mIsOtaPoorTimeOut && d_rssi > -5;
    ...
    private float checkGoodSpd(int rssi, long spd) {
        ...
        while (i < len) {
            if (rssi < RSSI_GOOD_SPD_TBL[i].RSSI_VAL) {
                ...
                float retScore = WifiProHistoryRecordManager.HOME_AP_LEVEL_THREE_SWITCH_RATE + ((((float) (spd - ((long) RSSI_GOOD_SPD_TBL[i].SPD_VAL))) * ADD_UNIT_SPD_SCORE) / ((float) RSSI_GOOD_SPD_TBL[i].UNIT_SPD_VAL));
    ...
    private void resetHighDataFlow() {
        this.mHighDataFlowRate = WifiProHistoryRecordManager.HOME_AP_DEFAULT_SWITCH_RATE;
    ...
    private int wpGoodLinkLevelCaculate(int mGoodLinkTgtRssi, int avgRssi, double RssiLastLoseRate) {
        ...
        if (avgRssi - mGoodLinkTgtRssi >= 8 || RssiLastLoseRate <= 0.05d) {
            mCneGoodLinkLevel = 5;
        } else if (avgRssi - mGoodLinkTgtRssi >= 5 || RssiLastLoseRate <= GOOD_LINK_LEVEL_4_BETTER_LOSERATE) {
            mCneGoodLinkLevel = 4;
        } else {
            mCneGoodLinkLevel = 3;
        }
    ...
    private void getCurrTcpRtt(ProcessedTcpResult mTcpResult) {
        ...
        if (tmpTcr != null) {
            long dtime = (nowtime - tmpTcr.mUpdateTime) + ((long) (tmpTcr.mTcpRttWhen * HwWifiStateMachine.AP_CAP_CACHE_COUNT));

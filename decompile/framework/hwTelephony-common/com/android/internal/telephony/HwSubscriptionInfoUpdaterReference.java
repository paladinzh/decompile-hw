...
public class HwSubscriptionInfoUpdaterReference implements SubscriptionInfoUpdaterReference {
    ...
    public void updateIccAvailability(int slotId) {
        ...
        if (mUiccController != null) {
            ...
            if (PROJECT_SIM_NUM <= 1 || subHelper.proceedToHandleIccEvent(slotId)) {
                ...
                boolean isNotTwoModemAndRadioNotAvailable;
                Rlog.i(LOG_TAG, "updateIccAvailability: newCard is null, slotId " + slotId);
                if (HwVSimUtils.isPlatformTwoModems()) {
                    isNotTwoModemAndRadioNotAvailable = HwVSimUtils.isRadioAvailable(slotId);
                } else {
                    isNotTwoModemAndRadioNotAvailable = true;
                }
                ...
                if (!IccCardStatusUtils.isCardPresent(newState)) {
                    ...
                    if (HwVSimUtils.needBlockUnReservedForVsim(slotId) && !IccCardStatusUtils.isCardPresent(newState)) {
                        isNeedUpdateForUnReservedVSimSub = IccCardStatusUtils.isCardPresent(oldState);
                    }
    ...
    private void changeIccidForHotplug(int slotId, CardState[] cardState) {
        ...
        } else if (IccCardStatusUtils.isCardPresent(cardState[slotId]) || !isCardTrayOut(0)) {
    ...
    public void broadcastSubinfoRecordUpdated(String[] iccId, String[] oldIccId, int nNewCardCount, int nSubCount, int nNewSimStatus) {
        ...
        for (i = 0; i < PROJECT_SIM_NUM; i++) {
            hasSimRemoved = (iccId[i] == null || !iccId[i].equals(ICCID_STRING_FOR_NO_SIM)) ? false : !oldIccId[i].equals(ICCID_STRING_FOR_NO_SIM);
            if (hasSimRemoved) {
                break;
            }
        }

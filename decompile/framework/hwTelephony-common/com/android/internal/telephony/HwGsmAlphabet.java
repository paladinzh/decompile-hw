...
public class HwGsmAlphabet {
    ...
    public static String gsm8BitUnpackedToString(byte[] data, int offset, int length, String characterset) {
        ...
        while (i < offset + length) {
            ...
            if (c != 27) {
                ...
                } else if (!isMbcs || c < HwSmsMessage.SMS_TOA_UNKNOWN || i + 1 >= offset + length) {
                    ret.append(languageTableToChar.charAt(c));
                    i2 = i;
                } else {
                    mbcsBuffer.clear();
    ...
    static {
        ...
        for (i = 0; i < numTables; i++) {
            ...
            if (!(tableLen == 0 || tableLen == HwSmsMessage.SMS_TOA_UNKNOWN)) {
        ...
        for (i = 0; i < numShiftTables; i++) {
            ...
            if (!(shiftTableLen == 0 || shiftTableLen == HwSmsMessage.SMS_TOA_UNKNOWN)) {

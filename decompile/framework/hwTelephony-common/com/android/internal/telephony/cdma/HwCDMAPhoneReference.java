...
public class HwCDMAPhoneReference extends HwPhoneReferenceBase implements CDMAPhoneReference {
    ...
    public void globalEccCustom(String operatorNumeric) {
        ...
        if (!TextUtils.isEmpty(operatorNumeric)) {
            if (!(operatorNumeric.equals(this.preOperatorNumeric) && "invalid".equals(forceEccState))) {
                isValid = true;
                if (isValid) {
                    ...
                    custEcc = getCustEccList(operatorNumeric);
                    if (!TextUtils.isEmpty(custEcc)) {
                        custEccArray = custEcc.split(":");
                    ...
                    if (obj == null) {
                        cursor = this.mPhone.getContext().getContentResolver().query(GlobalMatchs.CONTENT_URI, new String[]{"_id", NumMatchs.NAME, "numeric", VirtualNets.ECC_WITH_CARD, VirtualNets.ECC_NO_CARD}, "numeric=\"" + operatorNumeric + "\"", null, NumMatchs.DEFAULT_SORT_ORDER);
                        if (cursor != null) {
                    ...
                    validEcclist = TextUtils.isEmpty(obj) || !TextUtils.isEmpty(obj2);
                    if (validEcclist) {
                        this.mPhone.mCi.requestSetEmergencyNumbers("", "");
                    } else {

...
public class HwDcTrackerBaseReference implements DcTrackerBaseReference {
    ...
    public boolean XCAP_IGNORE_ROAMING_FETURE_ON = false;
                    ...
                    if (location instanceof GsmCellLocation) {
                        ...
                        if (-1 == HwDcTrackerBaseReference.newRac) {
                            ...
                            if (!(!HwDcTrackerBaseReference.this.mDcTrackerBase.mUserDataEnabled || defaultApn == null || defaultApn.getState() == State.CONNECTED)) {
                                ...
                                DcTrackerUtils.cleanUpConnection(dcTracker, !isDisconnected, defaultApn);
    ...
    static {
        ...
        MMSIgnoreDSSwitchOnRoaming = z;
        if (((MMS_PROP >> 2) & 1) != 1) {
            z2 = false;
        }
        MMSIgnoreDSSwitchNotRoaming = z2;
    }
    ...
    public void init() {
        ...
        this.mDcTrackerBase.INTELLIGENT_DATA_SWITCH_CONFIG = SystemProperties.getBoolean("ro.hwpp.autodds", false);
    ...
    public boolean isDataAllowedByApnContext(ApnContext apnContext) {
        if (isGsmOnlyPsNotAllowed()) {
            ...
            if (isXcap && this.XCAP_IGNORE_ROAMING_FETURE_ON && failureReason.isFailForSingleReason(DataAllowFailReasonType.ROAMING_DISABLED)) {
                return true;
            }
            return dataAllowed;
        }
    ...
    public boolean isDataAllowedByApnType(DataAllowFailReason failureReason, String apnType) {
        ...
        if (isXcap && this.XCAP_IGNORE_ROAMING_FETURE_ON && failReason.isFailForSingleReason(DataAllowFailReasonType.ROAMING_DISABLED)) {
            return true;
        }
        return dataAllowed;
    }
    ...
    private boolean isGsmOnlyPsNotAllowed() {
        ...
        if (isMultiSimEnabled) {
            if (!(TelephonyManager.getTelephonyProperty(this.mDcTrackerBase.mPhone.getPhoneId(), "gsm.data.gsm_only_not_allow_ps", "false").equals("true") && 1 == this.nwMode)) {
                z = false;
            }
            return z;
        }
    ...
    public boolean getAnyDataEnabledByApnContext(ApnContext apnContext, boolean enable) {
        ...
        if (!this.mDcTrackerBase.mPhone.getServiceState().getRoaming()) {
            ...
            return z;
        } else if (this.XCAP_IGNORE_ROAMING_FETURE_ON && "xcap".equals(apnContext.getApnType()) && this.mDcTrackerBase.isDataEnabled(false)) {
            return true;
        } else {
    ...
    public boolean noNeedDoRecovery(ConcurrentHashMap mApnContexts) {
        ...
        if (!SystemProperties.getBoolean("hw.ds.np.nopollstat", true) || isActiveDefaultApnPreset(mApnContexts)) {
            return false;
        }
        return true;
    }
    ...
    protected void onActionIntentSetNetworkType(Intent intent) {
        ...
        DcTrackerUtils.cleanUpConnection(this.mDcTrackerBase, !isDisconnected, defaultApn);
    ...
    public void disposeAddedIntent(String action) {
        if ("android.intent.action.INTELLGENT_DATA_SWITCH_IS_ON".equals(action)) {
            this.mDcTrackerBase.mIntelligentDataSwitchIsOn = true;
            ...
            this.mDcTrackerBase.mIntelligentDataSwitchIsOn = false;
    ...
    public void setXcapDataRoamingEnable(IccRecords r) {
        int i = 0;
        this.XCAP_IGNORE_ROAMING_FETURE_ON = false;
        String plmnsConfig = System.getString(this.mDcTrackerBase.mPhone.getContext().getContentResolver(), "xcap_roaming_enable_plmn");
        ...
        if (!TextUtils.isEmpty(plmnsConfig) && !TextUtils.isEmpty(operator)) {
            ...
            while (i < length) {
                ...
                if (plmn == null || !plmn.equals(operator)) {
                    ...
                    log("setXcapDataRoamingEnable: xcap roaming enable for operator " + operator);
                    this.XCAP_IGNORE_ROAMING_FETURE_ON = true;
                    return;
                }
            }
        }
    ...
    public boolean getXcapDataRoamingEnable() {
        return this.XCAP_IGNORE_ROAMING_FETURE_ON;
    }
}

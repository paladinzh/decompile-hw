...
public class HwAllInOneController extends Handler {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("android.intent.action.ACTION_SUBINFO_RECORD_UPDATED".equals(intent.getAction())) {
                ...
                if (HwAllInOneController.IS_FAST_SWITCH_SIMSLOT) {
                    HwAllInOneController.this.sendResponseToTarget(HwAllInOneController.this.obtainMessage(HwAllInOneController.EVENT_FAST_SWITCH_SIM_SLOT_DONE, Integer.valueOf(HwAllInOneController.this.expectedDDSsubId)), 0);
                } else {
                ...
                if (HwAllInOneController.IS_FAST_SWITCH_SIMSLOT) {
                    ...
                    HwAllInOneController.this.sendResponseToTarget(HwAllInOneController.this.mSet4GSlotCompleteMsg, 2);
                } else {
                    Message response = HwAllInOneController.this.obtainMessage(10, Integer.valueOf(HwAllInOneController.this.expectedDDSsubId));
    ...
    static {
        ...
        if (IS_FULL_NETWORK_SUPPORTED && !HwModemCapability.isCapabilitySupport(9) && SystemProperties.getBoolean("persist.hisi.fullnetwork", true)) {
            equals = "normal".equals(SystemProperties.get("ro.runmode", "normal"));
        } else {
            equals = false;
        }
        IS_FULL_NETWORK_SUPPORTED_IN_HISI = equals;
        equals = (HwModemCapability.isCapabilitySupport(9) || !"normal".equals(SystemProperties.get("ro.runmode", "normal"))) ? false : !IS_CHINA_TELECOM;
        IS_HISI_DSDX = equals;
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case 3:
                ...
                ProxyController.getInstance().stopTransaction();
                break;
            case 10:
                if (hasMessages(14)) {
                    removeMessages(14);
                ...
                if (this.mSet4GSlotCompleteMsg == null && this.mEventsQ.size() == 0) {
            ...
            case EVENT_RADIO_AVAILABLE /*104*/:
                ...
                if (SIM_NUM == 2 && !this.isVoiceCallEndedRegistered) {
            ...
            case EVENT_VOICE_CALL_ENDED /*113*/:
                ...
                if (SubscriptionController.getInstance().getSubState(index.intValue() == 0 ? 1 : 0) != 1 && SubscriptionController.getInstance().getSubState(index.intValue()) == 1) {
            ...
            case EVENT_FAST_SWITCH_SIM_SLOT_DONE /*114*/:
                ...
                if (ar == null || ar.exception != null) {
                    ...
                    this.mContext.sendBroadcast(new Intent("com.huawei.action.ACTION_HW_SWITCH_SLOT_DONE"));
    ...
    private void restartRildForNvcfg() {
        if (!this.mNvRestartRildDone && this.mAutoSwitchDualCardsSlotDone && this.mSetSdcsCompleteMsg == null && "1".equals(SystemProperties.get("gsm.nvcfg.resetrild", "0"))) {
            if (!(!IS_FULL_NETWORK_SUPPORTED_IN_HISI || HwFullNetwork.getInstance().getNeedSwitchCommrilMode() || IS_FAST_SWITCH_SIMSLOT)) {
    ...
    private void switchDualCardsSlotIfNeeded() {
        ...
        if (!SystemProperties.getBoolean("persist.sys.dualcards", false)) {
            isCompleted = true;
        } else if (this.mAutoSwitchDualCardsSlotDone) {
            isCompleted = isBalongSimSynced();
        } else {
            isCompleted = false;
        }
        if (isCompleted) {
    ...
    private void processSubInfoRecordUpdated(int detectedType) {
        if (4 != detectedType) {
            ...
            boolean is4GState;
            if (IS_FULL_NETWORK_SUPPORTED || IS_CMCC_CU_DSDX_ENABLE) {
                is4GState = true;
            } else {
                is4GState = IS_CMCC_4G_DSDX_ENABLE;
            }
            if (is4GState) {
                if (this.isSet4GSlotInProgress) {
                ...
                boolean set4GDefaltSlot = (userPref4GSlot >= SIM_NUM || this.isSimInsertedArray[userPref4GSlot]) ? userPref4GSlot >= SIM_NUM : true;
                boolean need4GCheckWhenBoot = IS_CMCC_4GSWITCH_DISABLE && this.subCarrierTypeArray[0].isCMCCCard() != this.subCarrierTypeArray[1].isCMCCCard();
                if (set4GDefaltSlot) {
                    judgeDefalt4GSlot();
    ...
    private void processSimStateChanged(String simState, int slotId) {
        if (slotId < 0 || SIM_NUM <= slotId) {
            ...
            if (oldSubCarrierTypeArr[slotId] == this.subCarrierTypeArray[slotId] || this.subCarrierTypeArray[slotId].isReCheckFail()) {
                isSubCarrierTypeChangedForReady = false;
            } else {
            ...
            if (isSubCarrierTypeChangedForReady) {
                ...
                if (2 == this.is4GSlotReviewNeeded && !this.isSet4GSlotInProgress) {
                    this.retryChangeCount = 0;
                    for (sub = 0; sub < SIM_NUM; sub++) {
                        if (this.mCis[sub] != null) {
                            this.mCis[sub].getPreferredNetworkType(obtainMessage(11, sub, 0));
    ...
    private String getIccId(int slot) {
        ...
        List<SubscriptionInfo> subInfo = SubscriptionController.getInstance().getSubInfoUsingSlotIdWithCheck(slot, false, this.mContext.getOpPackageName());
        if (subInfo != null) {
    ...
    public void switchDualCardsSlot(int slot, Message onCompleteMsg) {
        ...
        if (HwVSimUtils.isVSimEnabled() || HwVSimUtils.isVSimCauseCardReload() || HwVSimUtils.isSubActivationUpdate()) {
            isVSimRelated = true;
        } else if (HwVSimUtils.isAllowALSwitch()) {
            isVSimRelated = false;
        } else {
            isVSimRelated = true;
        }
    ...
    public boolean isSwitchDualCardSlotsEnabled() {
        boolean z = false;
        Rlog.d(this.TAG, "mUiccCardSwitchTypes[0] = " + this.mSwitchTypes[0] + ", mUiccCardSwitchTypes[1] = " + this.mSwitchTypes[1]);
        ...
        if (IS_CHINA_TELECOM) {
            ...
            if (this.mSwitchTypes[0] == 1 && this.mSwitchTypes[1] == 1 && HwForeignUsimForTelecom.IS_OVERSEA_USIM_SUPPORT && !HwForeignUsimForTelecom.getInstance().isDomesticCard(this.mBalongSimSlot)) {
        ...
        refreshCardState();
        if (IS_CMCC_4G_DSDX_ENABLE && IS_CMCC_4GSWITCH_DISABLE && ((this.subCarrierTypeArray[0].isCMCCCard() && !this.subCarrierTypeArray[1].isCMCCCard()) || (this.subCarrierTypeArray[1].isCMCCCard() && !this.subCarrierTypeArray[0].isCMCCCard()))) {
            return false;
        }
        if (this.mSwitchTypes[0] > 0 || this.isSimInsertedArray[0]) {
            if (this.mSwitchTypes[1] <= 0) {
                z = this.isSimInsertedArray[1];
            } else {
                z = true;
            }
        }
    ...
    private void recordPrimaryAndSecondaryStackNetworkType(int ability, int lteOnMappingMode) {
        ...
        if (isCmccHybird) {
            if (this.mPrimaryStackPhoneId == this.default4GSlot) {
                this.mSecondaryStackNetworkType = ability == 1 ? 25 : 24;
            } else {
                this.mPrimaryStackNetworkType = ability == 1 ? 25 : 24;
            }

...
public class HwVSimDWorkProcessor extends HwVSimWorkProcessor {
    ...
    protected void onSwitchSlotDone(Message msg) {
        ...
        if (isAsyncResultValidForRequestNotSupport(ar) && this.mModemAdapter.isDoneAllSwitchSlot(this, ar)) {
            ...
            this.mModemAdapter.getRadioCapability(subId);
    ...
    protected void onCardPowerOnDone(Message msg) {
        ...
        if (isAsyncResultValidNoProcessException(ar)) {
            ...
            int subId = request.mSubId;
            int subCount = request.getSubCount();
            boolean needRadioPowerOn = true;
            int appropriateIndex = -1;
            for (int i = 0; i < subCount; i++) {
                if (subId == request.getSubIdByIndex(i)) {
                    request.setCardOnOffMark(i, false);
                    appropriateIndex = i;
                    needRadioPowerOn = isNeedRadioPowerOn(request, subId);
                }
            }
            ...
            if (needRadioPowerOn) {
                ...
                if (appropriateIndex != -1) {
                    request.setPowerOnOffMark(appropriateIndex, false);
                }
                if (isAllMarkClear(request)) {
                    logd("onCardPowerOnDone: isAllMarkClear -> disableVSimDone");
                    disableVSimDone();
                }
            }
        }
    ...
    private boolean isNeedRadioPowerOn(HwVSimRequest request, int subId) {
        boolean isNeedSwitchCommrilMode = !(this.mVSimController.getSubState(subId) != 0) ? request.getIsNeedSwitchCommrilMode() : true;
        logd("isNeedRadioPowerOn: slot[" + subId + "] need power on: " + isNeedSwitchCommrilMode);
        return isNeedSwitchCommrilMode;
    }
}

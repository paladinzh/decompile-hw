...
public class HwVSimController extends StateMachine {
    ...
    private int mAutoSwitchSimSlotTimes = 0;
    ...
    private int getModemSupportVSimVersionInner() {
        ...
        int version = ((Integer) sendRequest(29, null, 0)).intValue();
    ...
    public boolean needBlockUnReservedForVsim(int subId) {
        if (!(HwVSimUtilsInner.isPlatformRealTripple() || isPlatformTwoModems()) || HwVSimUtilsInner.isVSimDsdsVersionOne()) {
    ...
    public int writeVsimToTA(String imsi, int cardType, int apnType, String challenge, String taPath, int vsimLoc, int modemID) {
        ...
        if (challenge != null && challenge.length() != 0) {
            return new HwVSimOperateTA().operateTA(1, imsi, cardType, apnType, challenge, false, taPath, vsimLoc, modemID);
        }
    ...
    public int clearVsimToTA() {
        logd("clearVsimToTA");
        return new HwVSimOperateTA().operateTA(3, null, 0, 0, null, false, null, 0, 0);
    }
    ...
    private int writeApnToTA(String imsi, int cardType, int apnType, String tapath, String challenge, boolean isForHash) {
        logi("writeApnToTA");
        if (challenge != null && challenge.length() != 0) {
            return new HwVSimOperateTA().operateTA(2, imsi, cardType, apnType, challenge, isForHash, tapath, 0, 0);
        }
    ...
    private boolean needSwitchModeHotplug(int[] cardTypes) {
        ...
        boolean isM2hasSimAndM0IsULG = (!hasIccCardOnM2(cardTypes) || getULOnlyProp()) ? false : getVSimULOnlyMode();
        if (isM2hasSimAndM0IsULG) {
            ...
            if (!getVSimULOnlyMode()) {
                int reservedSub = getUserReservedSubId();
                if (oldInsertedCardCount == 2 && newInsertedCardCount == 2 && reservedSub != -1) {
            ...
            return checkIfHasCdmaCard(cardTypes, expect, mainSlot) || expect != current;
        } else {
    ...
    private void onAutoSwitchSimSlot() {
        logd("onAutoSwitchSimSlot, mAutoSwitchSimSlotTimes = " + this.mAutoSwitchSimSlotTimes);
        if (HwAllInOneController.IS_HISI_DSDS_AUTO_SWITCH_4G_SLOT) {
            ...
            if (default4GSlotId != activatedSlotId) {
                ...
                this.mAutoSwitchSimSlotTimes = 0;
                HwAllInOneController.getInstance().setDefault4GSlot(activatedSlotId, null);
            } else {
                this.mAutoSwitchSimSlotTimes++;
                if (5 <= this.mAutoSwitchSimSlotTimes) {
                    this.mAutoSwitchSimSlotTimes = 0;
                    logd("onAutoSwitchSimSlot: has retry for 5times, return.");
                    return;
                }
                this.mMainHandler.sendEmptyMessageDelayed(4, 500);
            }
        }
    }
    ...
    private int getSwitchActivatedSubId(int oldActivatedSubId) {
        ...
        if (HwAllInOneController.IS_HISI_DSDS_AUTO_SWITCH_4G_SLOT) {
            ...
            for (int i = 0; i < this.mCis.length; i++) {
                if (subController.getSubState(subController.getSubId(i)[0]) == 1) {
    ...
    private boolean isSubActivedDone() {
        synchronized (sCheckSubActivedLock) {
            int i;
            for (i = 0; i < this.mCheckSubActivated.length; i++) {
                if (this.mCheckSubActivated[i]) {

...
public class HwForeignUsimForTelecom extends Handler {
    ...
    public void checkIfAllCardsReady() {
        ...
        if (!HwModemCapability.isCapabilitySupport(9)) {
            ...
            boolean atLeastOneSlotHasSim;
            if (-1 == this.mOldMainSwitchTypes[0] && -1 != this.mSwitchTypes[0] && this.mSwitchTypes[0] != 0) {
                atLeastOneSlotHasSim = true;
            } else if (-1 != this.mOldMainSwitchTypes[1] || -1 == this.mSwitchTypes[1]) {
                atLeastOneSlotHasSim = false;
            } else {
                if (this.mSwitchTypes[1] != 0) {
                    z = true;
                }
                atLeastOneSlotHasSim = z;
            }
    ...
    private void trySwitchCommrilMode() {
        ...
        if (!(-1 == foreignUCardSlot || IS_OVERSEAS_MODE || this.mRatCombineMode[this.mBalongSimSlot] != 0 || isDomesticCard(this.mMainSlot))) {
    ...
    public boolean isDomesticCard(int slotId) {
        ...
        boolean isCMCCCard;
        if (!TextUtils.isEmpty(this.mIccIds[slotId]) && PREFIX_MII_ICCID.equals(this.mIccIds[slotId].substring(0, 2))) {

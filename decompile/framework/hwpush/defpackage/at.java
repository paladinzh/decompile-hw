...
public class at extends o {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if ("com.huawei.android.push.intent.HEARTBEAT_RANGE_CHANGE".equals(action) || "com.huawei.android.push.intent.HEARTBEAT_VALID_ARRIVED".equals(action)) {
            try {
                aw.d("PushLog2841", "when receive the heart beat range change or valid arrived,the file  HeartBeatCfg.xml need del");
                for (String action2 : ChannelMgr.g(context).aT()) {
                    au.n(context, action2);
                }
            } catch (Exception e) {
                aw.d("PushLog2841", "when receive the heart beat range change, files delete failed!");
            }
        }
    }
}

...
public class bl {
    private static void a(Context context, int i, HashMap hashMap) {
        if (context == null || hashMap == null) {
            aw.i("PushLog2841", "startMonitor, map is null");
            return;
        }
        aw.i("PushLog2841", "startMonitor, eventId is " + i);
        String string = new bt(context, "PushMonitor").getString(String.valueOf(i));
        al alVar = new al(86400000, 2);
        alVar.i(string);
        if (alVar.k(1)) {
            aw.i("PushLog2841", "begin to startMonitor");
            new Thread(new bm(context, alVar, i, hashMap)).start();
            return;
        }
        aw.i("PushLog2841", "can't report too many times");
    }
    ...
    private static void a(Context context, al alVar, int i, HashMap hashMap) {
        try {
            ...
            if (invoke != null) {
                ...
                alVar.l(1);
                new bt(context, "PushMonitor").f(String.valueOf(i), alVar.bH());
            }
        } catch (ClassNotFoundException e) {
            aw.e("PushLog2841", " ClassNotFoundException startMonitor " + e.toString());
        } catch (NoSuchMethodException e2) {
            aw.e("PushLog2841", " NoSuchMethodException startMonitor " + e2.toString());
        } catch (IllegalArgumentException e3) {
            aw.e("PushLog2841", " IllegalArgumentException startMonitor " + e3.toString());
        } catch (IllegalAccessException e4) {
            aw.e("PushLog2841", " IllegalAccessException startMonitor " + e4.toString());
        } catch (Exception e5) {
            aw.e("PushLog2841", " Exception startMonitor " + e5.toString());
        }
    }

    public static void ad(Context context) {
        HashMap hashMap = new HashMap();
        hashMap.put(Short.valueOf((short) 0), String.valueOf(2841));

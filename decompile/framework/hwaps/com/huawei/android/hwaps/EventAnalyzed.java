...
public class EventAnalyzed implements IEventAnalyzed {
    ...
    private void increaseFPS(int fps) {
        if (this.mIsControllAllFpsEnable || (this.mIsAnalyzedEnable && (this.mGameType == 8 || this.mGameType == 9 || this.mGameType == 10 || this.mGameType == 11))) {
            if (55 == fps) {
        ...
        if (-1 != this.mLastUpTime) {
            ...
            if (this.mConsecutiveShortDownUpCount > 5) {
                if (this.mCanUseDebug && this.mSdrController != null && this.mSdrController.IsSdrCase()) {
                    ...
                    if (this.mAverageFps < 10 || this.mIsLowFps || this.mStatisFpsCount < 50) {
    ...
    private void analyzeUpAction(long eventTime, long downTime, int x, int y) {
        ...
        if ((this.mXScrollCount < 3 || this.mYScrollCount <= 0) && ((this.mYScrollCount < 3 || this.mXScrollCount <= 0) && this.mXScrollCount < 4)) {
            ...
            if (this.mDownMoveInterval >= 1000 && !this.isXScroll && !this.isYScroll) {
    ...
    private int analyzeGameType(int shortAndLongTouchProportion, int totalTouchCount) {
        ...
        } else if (this.mClickCount < 3 && !this.mIsScreenLandScape && this.mSlowScrollCount < 3) {
    ...
    private void sendBroadcastBaseOnAction(final Context context, final int action) {
        new Thread(new Runnable() {
            public void run() {
                if (action == 0) {
                    ApsCommon.logD(EventAnalyzed.TAG, "APK: APS: 30Hz: ACTION_DOWN");
                    context.sendBroadcast(new Intent(EventAnalyzed.APS_TOUCH_ACTION_DOWN));
                } else if (action == 1) {
                    ApsCommon.logD(EventAnalyzed.TAG, "APK: APS: 30Hz: ACTION_UP");
                    context.sendBroadcast(new Intent(EventAnalyzed.APS_TOUCH_ACTION_UP));
                }
            }
        }).start();
    }

    private void sendBroadcastBaseOnGame(final int mode) {
        new Thread(new Runnable() {
            public void run() {
    ...
    public void processAnalyze(Context context, int action, long eventTime, int x, int y, int pointCount, long downTime) {
        ...
        if (9000 == SystemProperties.getInt("debug.aps.enable", 0)) {
            ...
            if (action != 0 || this.mTouchResumeEnable) {
                if (action == 1 && this.mTouchResumeEnable) {
            ...
            if (eventTime - this.mBeginAnalyzeTime <= ANALYZE_TIME) {
                ...
                if (openGLType == 10 || (openGLType == 11 && !this.mIsScreenLandScape)) {
                    gametype = openGLType;
                }
    ...
    public void setAdaptSdr(int gameType, int openGLType) {
        ...
        if (!this.mCanUseDebug && CUST_GAME_TYPE == this.mGameType && getCurrentBattery() <= getCtrlBattery(this.mPkgName) && this.mSdrController.IsSdrCase()) {
            ...
            if ((gameType == 10 || gameType == 8 || gameType == 9) && this.mIsAnalyzedEnable && !this.mCanUseDebug && this.mAverageFps >= 10 && !this.mIsLowFps && this.mStatisFpsCount >= 50) {

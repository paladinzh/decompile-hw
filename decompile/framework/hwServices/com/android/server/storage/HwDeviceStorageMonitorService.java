...
public class HwDeviceStorageMonitorService extends DeviceStorageMonitorService {
    ...
    public void sendNotificationHwSM(long freeMem, boolean isBootImageOnDisk, int notficationId, Intent storageLowIntent) {
        ...
        Notification notification = new Builder(context).setSmallIcon(33751342).setLargeIcon(BitmapFactory.decodeResource(context.getResources(), 33751342)).setTicker(title).setColor(context.getColor(17170519)).setContentTitle(title).setContentText(details).setContentIntent(PendingIntent.getActivityAsUser(context, 0, lowMemIntent, 0, null, UserHandle.CURRENT)).setStyle(new BigTextStyle().bigText(details)).setVisibility(1).setCategory("sys").setExtras(HwNotificationResource.getNotificationThemeData(33751343, -1, 2, 15)).build();

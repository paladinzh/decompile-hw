...
public final class HwActivityStackSupervisor extends ActivityStackSupervisor {
    ...
    ResolveInfo resolveIntent(Intent intent, String resolvedType, int userId, int flags) {
        ...
        if (!this.mService.mUserController.getUserManagerInternal().isSameGroupForClone(callingUserId, userId)) {
            return super.resolveIntent(intent, resolvedType, userId, flags);
        }
    ...
    protected boolean isInMultiWinBlackList(String pkgName) {
        if (IS_TABLET) {
            if ("com.huawei.camera".equals(pkgName)) {
                return true;
            }
        } else if ("com.huawei.systemmanager".equals(pkgName)) {
            return true;
        }
        return false;
    }

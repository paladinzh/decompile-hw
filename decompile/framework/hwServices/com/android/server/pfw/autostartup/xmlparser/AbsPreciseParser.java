...
abstract class AbsPreciseParser implements IStartupParser<PreciseComponent> {
    ...
    public PreciseComponent parseDOMElement(Element ele) {
        PreciseComponent result = new PreciseComponent(getPreciseType(), ele.getAttribute("name"), parseScope(ele.getAttribute(PreciseIgnore.COMP_COMM_SCOPE_ATTR)), parseScreenStatus(ele.getAttribute(PreciseIgnore.COMP_COMM_SCREEN_ATTR)));

...
public class HwNetworkPropertyChecker {
        ...
        public void run() {
            ...
            try {
                ...
                if (connection instanceof HttpURLConnection) {
                    ...
                    if (!(HwNetworkPropertyChecker.this.mIgnoreRxCounter || httpResponseCode != WifiProCommonUtils.HTTP_UNREACHALBE || HwNetworkPropertyChecker.this.mNetworkDisconnected.get() || HwNetworkPropertyChecker.this.mNetwork == null || HwNetworkPropertyChecker.this.mInOversea || (!this.reconfirm && HwNetworkPropertyChecker.this.mCheckingCounter != HwNetworkPropertyChecker.this.mMaxAttempts))) {
                        try {
                ...
                if (msg == null || !msg.contains("ECONNREFUSED") || HwNetworkPropertyChecker.this.mCurrentWifiConfig == null || !WifiProCommonUtils.matchedRequestByHistory(HwNetworkPropertyChecker.this.mCurrentWifiConfig.internetHistory, 100)) {
                    if (msg != null) {
                        if (msg.contains("ECONNRESET") && HwNetworkPropertyChecker.this.mCurrentWifiConfig != null && WifiProCommonUtils.matchedRequestByHistory(HwNetworkPropertyChecker.this.mCurrentWifiConfig.internetHistory, 100)) {
                            ...
                            WifiProCHRManager.getInstance().updateWifiException(122, WifiProCHRManager.SUB_EVENT_ACTIVE_CHECK_FAIL);
                        }
                    }
                    if (msg != null) {
            ...
            if (WifiProCommonUtils.isRedirectedRespCode(respCode)) {
                ...
                if (!(host == null || newLocation == null || HwNetworkPropertyChecker.this.mCurrentWifiConfig == null)) {
                    ...
                    if ((HwNetworkPropertyChecker.CHINA_MAINLAND_MAIN_SERVER.equals(server) && host.contains(SERVER_BAIDU)) || ((HwNetworkPropertyChecker.CHINA_MAINLAND_BACKUP_SERVER.equals(server) && host.contains(SERVER_YOUKU)) || ((HwNetworkPropertyChecker.OVERSEA_BACKUP_SERVER.equals(server) && host.contains(SERVER_BING)) || (HwNetworkPropertyChecker.OVERSEA_BACKUP_YOUTUBE.equals(server) && host.contains(SERVER_YOUTUBE))))) {
                        ...
                        WifiProCHRManager.getInstance().updateWifiException(122, WifiProCHRManager.SUB_EVENT_NOT_OPEN_AP_REDIRECT);
                    }
                }
    ...
    public static class StarndardPortalInfo {
        public String currentSsid = AppHibernateCst.INVALID_PKG;
        public int lac = -1;
    ...
    private void sendNetworkCheckingStatus(String action, String flag, int property) {
        ...
        if (property == 6 && !TextUtils.isEmpty(this.mRawRedirectedHostName)) {
    ...
    public boolean isCheckingCompleted(int finalRespCode) {
        ...
        if (WifiProCommonUtils.httpReachableOrRedirected(finalRespCode)) {
            ...
            if (!(this.mControllerNotified || isNetworkChanged())) {
                ...
                property = -1;
                if (this.mCurrentWifiConfig != null) {
                    property = (!WifiProCommonUtils.matchedRequestByHistory(this.mCurrentWifiConfig.internetHistory, 100) || isNetworkPoorRssi() || WifiProCommonUtils.isOpenType(this.mCurrentWifiConfig)) ? -1 : NETWORK_PROPERTY_PENDING;
                }
                sendNetworkCheckingStatus(ACTION_NETWOR_PROPERTY_NOTIFICATION, EXTRA_FLAG_NETWOR_PROPERTY, property);
    ...
    private int recomfirmResponseCode(int respCode, boolean reconfirm) {
        ...
        if ((!this.mInOversea || useBackupSvr) && WifiProCommonUtils.isOpenType(this.mCurrentWifiConfig) && WifiProCommonUtils.isInMonitorList(this.mCurrentWifiConfig.SSID, SPECIAL_REDIRECTED_PORTAL_LIST)) {
            respCode = 302;
            if (syncRecheckBasedOnWebView(this.mInOversea) == 101 && !this.mNetworkDisconnected.get()) {
                LOGW("recomfirmRespCode, SPECIAL_PORTAL, connected network type is hasInternet based on the WebView loading results.");
                repairOk = true;
                newRespCode = 200;
            }
        } else if (respCode == WifiProCommonUtils.RESP_CODE_UNSTABLE || ((respCode == WifiProCommonUtils.RESP_CODE_GATEWAY && WifiProCommonUtils.isOpenType(this.mCurrentWifiConfig)) || respCode == WifiProCommonUtils.RESP_CODE_INVALID_URL || respCode == WifiProCommonUtils.RESP_CODE_ABNORMAL_SERVER)) {
        ...
        } else if ((!this.mInOversea || useBackupSvr) && WifiProCommonUtils.httpReachableHome(newRespCode) && WifiProCommonUtils.isOpenType(this.mCurrentWifiConfig) && !this.mForcedNoRecomfirm && !this.mMobileHotspot && !isStarndardPortalAuthenCompleted(newRespCode)) {
    ...
    private int recheckForAbnormalStatus(int respCode, boolean reconfirm) {
        ...
        if (this.mNetworkDisconnected.get()) {
            ...
            return WifiProCommonUtils.HTTP_UNREACHALBE;
        } else if (type == 102 && respCode != WifiProCommonUtils.RESP_CODE_ABNORMAL_SERVER) {

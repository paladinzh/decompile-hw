...
public class HwPowerManagerService extends PowerManagerService {
                ...
                if (btClass != null) {
                    ...
                    if (needWakeUp) {
                        synchronized (HwPowerManagerService.this.mLock) {
                            if (HwPowerManagerService.this.wakeUpNoUpdateLocked(now, "bluetooth.connected", 1000, HwPowerManagerService.this.mContext.getOpPackageName(), 1000)) {
    ...
    private final class HeadsetReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            synchronized (HwPowerManagerService.this.mLock) {
                ...
                if (intent.getIntExtra("state", 0) == 1 && HwPowerManagerService.this.wakeUpNoUpdateLocked(now, "headset.connected", 1000, HwPowerManagerService.this.mContext.getOpPackageName(), 1000)) {
                    HwPowerManagerService.this.updatePowerStateLocked();
                }
            }
        }
    ...
    protected boolean acquireProxyWakeLock(IBinder lock, int flags, String tag, String packageName, WorkSource ws, String historyTag, int uid, int pid) {
        if (this.mSystemReady) {
            ...
            synchronized (this.mLock) {
                for (int i = 0; i < this.mProxyWLProcessList.size(); i++) {

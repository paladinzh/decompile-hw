...
public class HwGpsLogServices implements IHwGpsLogServices {
    ...
    private boolean mGeolocationCollectEnable = true;
    HwGnssDataCollector mGnssDataCollector;
    HwGpsSessionRecorder mHwGpsSessionRecorder;
    HwOldGpsLogServices mHwOldGpsLogServices;
    private boolean mNewGpsChrEnable = SystemProperties.getBoolean("ro.config.hw_nc_chr_gps", true);
    ...
    public int logEvent(int type, int event, String parameter) {
        int result = -1;
        if (this.mNewGpsChrEnable) {
            result = this.mHwGpsSessionRecorder.updatelogEvent(type, event, parameter);
        }
        if (result != 0) {

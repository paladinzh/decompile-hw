...
public class GnssConnectivityLogManager extends Handler {
    ...
    private String getCenumValue(Cenum obj) {
        String value = null;
        try {
            Field nameField = obj.getClass().getSuperclass().getDeclaredField("name");
    ...
    private byte[] getExInfo(ConnectivityLog connectivityLog) {
        if (!COUNTRY_CODE_CN.equalsIgnoreCase(SystemProperties.get("ro.product.locale.region", AppHibernateCst.INVALID_PKG)) || getExEventType(connectivityLog.mEventType) == -1) {
            return null;
        }
    ...
    private void saveExInfo(String strJson) {
        ...
        if (!COUNTRY_CODE_CN.equalsIgnoreCase(SystemProperties.get("ro.product.locale.region", AppHibernateCst.INVALID_PKG))) {
        ...
        } else if (getExEventType(eventNo) == -1) {
            if (HWFLOW) {
            ...
            ConnectivityLog connectivityLog = new ConnectivityLog(null, -1, -1, -1, null, -1);
            if (str.compareTo(SUBSYS_WIFI) == 0) {
    ...
    private void reportConnectivityException(ConnectivityLog connectivityLog) {
        ...
        if (!GnssLogManager.getInstance().isCommercialUser()) {
            ...
            model.chrLogComHeadModel = new GnssChrCommonInfo().getChrComHead(mContext, false);
            GnssLogManager.getInstance().reportAbnormalEvent(model, connectivityLog.mMetricID, 1);
            int eventType = connectivityLog.mEventType;
            if (eventType != 71 && eventType != GPS_SESSION_EVENT) {
                switch (connectivityLog.mMetricID) {
                    case 14:
                        sendMessageDelayed(obtainMessage(3, connectivityLog), 2000);
                        break;
                    case 15:
                        break;
                    default:
    ...
    private void reportAbnormalEventWithErrCode(ChrLogBaseModel logModel, int metricID, int level, int eventType, Date date, int msg_type, int errorCode) {
        if (!GnssLogManager.getInstance().isOverseaCommercialUser()) {
            ...
            if (msg_type == 1) {
                ...
                sendMessage(logModel, metricID, level, eventType, date, false, errorCode);
            } else if (msg_type == 2) {
    ...
    private void sendMessage(ChrLogBaseModel logModel, int metricID, int level, int eventType, Date date, boolean isDaily, int errorCode) {
        ...
        Message targetMsg = obtainMessage(1, new ConnectivityLog(logInfo, metricID, level, eventType, date, errorCode));
        if (isDaily) {

...
public class HwLocalLocationManager implements IHwLocalLocationManager {
    ...
    private boolean isCellIdExist(int id) {
        ...
        Cursor cursor = null;
        try {
            cursor = this.mLocalLocationDB.query(TABLE_CELLID_NAME, CELLID_COLUMNS, CELL_SELECTION, new String[]{Sha256Encrypt.Encrypt(id + AppHibernateCst.INVALID_PKG, AppHibernateCst.INVALID_PKG), this.mCurrentAreaCode + AppHibernateCst.INVALID_PKG});
    ...
    public Location queryLocFormCellIdTable(int id) {
        ...
        String[] selectionArgs = new String[]{cellidStrSHA256, this.mCurrentAreaCode + AppHibernateCst.INVALID_PKG};
        Cursor cursor = null;
        try {
    ...
    private void updatetLocToCellIdTable(Location loc, int id) {
        ...
        try {
            ...
            values.put(COLUMN_LOC_SOURC, loc.getProvider());
            values.put(COLUMN_TIME, Long.valueOf(System.currentTimeMillis()));
            this.mLocalLocationDB.update(TABLE_CELLID_NAME, values, CELL_SELECTION, new String[]{idStrSHA256 + AppHibernateCst.INVALID_PKG, this.mCurrentAreaCode + AppHibernateCst.INVALID_PKG});
        } catch (Exception e) {
            Log.e(TAG, "query Exception :" + e.getMessage());
        }
    }

...
public class FingerprintActionsListener implements PointerEventListener {
    ...
    public void onPointerEvent(MotionEvent motionEvent) {
        if (this.mDeviceProvisioned) {
            ...
            if (this.mIsSingleFlinger) {
                ...
                if (!(!this.mIsValidHiboardGesture || isSuperPowerSaveMode() || isInLockTaskMode() || this.mIsStatusBarExplaned)) {
    ...
    private void touchDownIsValidLazyMode(float pointX, float pointY) {
        ...
        if (this.mPolicy.mNavigationBarOnBottom) {
            boolean z2 = (pointY <= ((float) (this.realSize.y - HIT_REGION_TO_MAX_LAZYMODE)) || (pointX >= ((float) HIT_REGION_TO_MAX_LAZYMODE) && pointX <= ((float) (this.realSize.x - HIT_REGION_TO_MAX_LAZYMODE)))) ? false : !isNaviBarEnable();
            this.mIsValidLazyModeGesture = z2;
            if (!this.mIsValidLazyModeGesture) {
                invalidPointX = this.realSize.x / 2;
                invalidPointY = this.realSize.y;
                if (pointY <= ((float) (this.realSize.y - HIT_REGION_TO_MAX_HIBOARD)) || pointX == ((float) invalidPointX) || pointY == ((float) invalidPointY) || isNaviBarEnable() || pointX <= ((float) (HIT_REGION_TO_MAX_LAZYMODE * 4)) || pointX >= ((float) (this.realSize.x - (HIT_REGION_TO_MAX_LAZYMODE * 4)))) {
                    z = false;
                } else if (FrontFingerPrintSettings.FRONT_FINGERPRINT_NAVIGATION_TRIKEY != 0 && (FrontFingerPrintSettings.FRONT_FINGERPRINT_NAVIGATION_TRIKEY != 1 || this.mTrikeyNaviMode >= 0)) {
                    z = false;
                }
                this.mIsValidHiboardGesture = z;
            }
        } else {
            invalidPointY = this.realSize.y / 2;
            invalidPointX = this.realSize.x;
            this.mIsValidLazyModeGesture = false;
            if (pointX <= ((float) (this.realSize.x - HIT_REGION_TO_MAX_HIBOARD)) || pointX == ((float) invalidPointX) || pointY == ((float) invalidPointY) || isNaviBarEnable() || pointY <= ((float) (HIT_REGION_TO_MAX_LAZYMODE * 4)) || pointY >= ((float) (this.realSize.y - (HIT_REGION_TO_MAX_LAZYMODE * 4)))) {
                z = false;
            } else if (FrontFingerPrintSettings.FRONT_FINGERPRINT_NAVIGATION_TRIKEY != 0 && (FrontFingerPrintSettings.FRONT_FINGERPRINT_NAVIGATION_TRIKEY != 1 || this.mTrikeyNaviMode >= 0)) {
                z = false;
            }
    ...
    private boolean touchDownIsValidMultiWin(MotionEvent event) {
        ...
        updateRealSize();
        boolean ret = this.mPolicy.mNavigationBarOnBottom ? pointY0 > ((float) (this.realSize.y - navigation_bar_height)) && pointY1 > ((float) (this.realSize.y - navigation_bar_height)) : pointX0 > ((float) (this.realSize.x - navigation_bar_height)) && pointX1 > ((float) (this.realSize.x - navigation_bar_height));

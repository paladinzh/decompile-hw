...
*/
    ...
    private void init() {
        ...
        if (this.mContext != null) {
            ...
            if (this.mIsSupport) {
                ...
                this.mTriggerLazyMode = this.mThreshHoldsLazyMode * 3.0f;
                if (System.getIntForUser(this.mContext.getContentResolver(), "single_hand_screen_zoom", 1, ActivityManager.getCurrentUser()) == 1) {
                    z2 = true;
                } else {
                    z2 = false;
                }
    ...
    private void registerObserver() {
        ContentObserver slideObserver = new ContentObserver(this.mHandler) {
            public void onChange(boolean selfChange) {
                ...
                if (System.getIntForUser(SlideTouchEvent.this.mContext.getContentResolver(), "single_hand_screen_zoom", 1, ActivityManager.getCurrentUser()) == 1) {
                    z = true;
                } else {
                    z = false;
                }
                ...
                if (Secure.getIntForUser(SlideTouchEvent.this.mContext.getContentResolver(), "accessibility_display_magnification_enabled", 0, ActivityManager.getCurrentUser()) != 1) {
                    z2 = false;
                }
            ...
            switch (event.getAction()) {
                ...
                case 1:
                    if (!this.mIsValidGuesture) {
                        ...
                        break;
                    } else if ((distanceX > this.mTriggerLazyMode || distanceY > this.mTriggerLazyMode) && 1 == this.mContext.getResources().getConfiguration().orientation) {
    ...
    public static Rect getScreenshotRect(Context context) {
        ...
        if (1 == state) {
            return new Rect(0, (int) (((float) displayMetrics.heightPixels) * 0.25f), (int) (((float) displayMetrics.widthPixels) * SCALE), displayMetrics.heightPixels);
        }
        if (2 == state) {

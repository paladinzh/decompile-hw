...
public class HwAmbientCCTFilterAlgo {
    ...
    private void applyLightSensorMeasurement(long time, float cct) {
        this.mAmbientCCTRingBuffer.prune(time - TableJankBd.recordMAXCOUNT);
    ...
    private void updateAmbientCCT(long time) {
        ...
        this.mNextTransitionTime = Math.min(nextDescendTransition, nextAscendTransition);
    ...
    private float calculateDescendThresholdDelta(float amCCT) {
        ...
        for (PointF temp : this.mData.CCTDescendlinePoints) {
            ...
            if (amCCT < temp.x) {
                PointF temp2 = temp;
                if (temp.x > temp1.x) {
                    return Math.max((((temp.y - temp1.y) / (temp.x - temp1.x)) * (amCCT - temp1.x)) + temp1.y, HwCircleAnimation.SMALL_ALPHA);
                }
                if (!DEBUG) {
                    return HwCircleAnimation.SMALL_ALPHA;
                }
    ...
    private boolean decideToAscend(float ambientCCT) {
        ...
        if (!needToAscend || this.mAutoCCTIntervened) {
            return false;
        }
        return true;
    }
    ...
    private boolean decideToDescend(float ambientCCT) {
        ...
        if (!needToDescend || this.mAutoCCTIntervened) {
            return false;
        }
        return true;
    }

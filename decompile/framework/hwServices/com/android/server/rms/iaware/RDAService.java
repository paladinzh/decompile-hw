...
public class RDAService {
    ...
    private void syncFeatureStatus(Bundle bundle) {
        ...
        for (int index = 0; index < featureNum; index++) {
            boolean featureEnabled = bundle.getInt(featureTypes[index].name(), 0) == 1;
            if (this.mRFeatureManager.getFeatureStatus(featureTypes[index].ordinal()) != bundle.getInt(featureTypes[index].name(), 0)) {

...
public class IOStatsService extends Stub {
    ...
    private long mLastWrittenBytes = 0;
    private List<LifeTimeData> mLifeTimeAPendingList = new ArrayList();
    private List<LifeTimeData> mLifeTimeBPendingList = new ArrayList();
    private List<Integer> mListTimeAValueList = new ArrayList();
    private List<Integer> mListTimeBValueList = new ArrayList();
    ...
    private List<WriteBytesData> mWrittenPendingList = new ArrayList();
    ...
    protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        } else if (Binder.getCallingUid() > 1000 || !TestCase.test(this.mContext, pw, args)) {
    ...
    private void dumpLifeTime(String tag, PrintWriter pw) {
        if (DUMP_TYPE_FULL.equals(tag) || DUMP_TYPE_PENDING.equals(tag)) {
            ...
            if (DUMP_TYPE_FULL.equals(tag)) {
                lifeAList = this.mIOExceptionHandle.getAllLifeTimeInfo(2);
                lifeBList = this.mIOExceptionHandle.getAllLifeTimeInfo(3);
            } else {
                lifeAList = this.mLifeTimeAPendingList;
                lifeBList = this.mLifeTimeBPendingList;
            }
            dumpLifeTime(tag, 0, pw, lifeAList);
            dumpLifeTime(tag, 1, pw, lifeBList);
        }
    }
    ...
    private void readStatsFromKernel() {
        ...
        if (this.mIsServiceReady) {
            ...
            for (int index = 0; index < currentIOStats.size(); index++) {
    ...
    public void periodReadTask() {
        ...
        int lifetimeB = KernelIOStats.getHealthInformation(1);
        long currentTime = Utils.getShortDateFormatValue(System.currentTimeMillis());
        if (this.mListTimeAValueList.size() == 0 || ((Integer) this.mListTimeAValueList.get(this.mListTimeAValueList.size() - 1)).intValue() != lifetimeA) {
        ...
        if (this.mListTimeBValueList.size() == 0 || ((Integer) this.mListTimeBValueList.get(this.mListTimeBValueList.size() - 1)).intValue() != lifetimeB) {
    ...
    private void getLastLifeTimeInformation() {
        LifeTimeData lifeTimeA = this.mIOExceptionHandle.getLastLifeTimeData(2);
        if (lifeTimeA != null) {
            this.mListTimeAValueList.add(Integer.valueOf(lifeTimeA.mLifeTime));
        }
        LifeTimeData lifeTimeB = this.mIOExceptionHandle.getLastLifeTimeData(3);
        if (lifeTimeB != null) {
    ...
    private void handleOverFlowSizeHistory(boolean isForceWrite, SparseArray<IOStatsHistory> pendingHistoryMap, SparseArray<IOStatsHistory> overFlowHistoryMap, long currentTimeMillis) {
        ...
        for (int index = 0; index < overFlowHistoryMap.size(); index++) {
    ...
    private SparseArray<IOStatsHistory> removeOverFlowFromPending(int totalPendingBytes, SparseArray<IOStatsHistory> pendingHistoryMap) {
        ...
        int totalBytesNum = 0;
        for (index = 0; index < pendingHistoryMap.size(); index++) {
        ...
        for (index = 0; index < overFlowHistoryMap.size(); index++) {
    ...
    public void saveIOStatsAndLatestUids(boolean isForceWrite) {
        if (!this.mIsServiceReady) {
            ...
            this.mIOExceptionHandle.saveLifeTime(this.mLifeTimeAPendingList, 2);
            this.mIOExceptionHandle.saveLifeTime(this.mLifeTimeBPendingList, 3);
    ...
    private Object[] getUidFromIntentExtras(Intent intent) {
        ...
        if (!"android.intent.action.UID_REMOVED".equals(intent.getAction())) {
            ...
            result[0] = Integer.valueOf(uid);
            result[1] = AppHibernateCst.INVALID_PKG;
            return result;
        } else {

...
public final class HwBatteryService extends BatteryService {
    ...
    private static final boolean isChinaRegion = SystemProperties.get("ro.config.hw_optb", PPPOEStateMachine.PHASE_DEAD).equals("156");
    private AudioAttributes mAudioAttributes;
    private final UEventObserver mBatteryFcpObserver = new UEventObserver() {
        public void onUEvent(UEvent event) {
            String fcpStatus = event.get("POWER_SUPPLY_FCP_STATUS", PPPOEStateMachine.PHASE_DEAD);
            String scpStatus = event.get("POWER_SUPPLY_SCP_STATUS", PPPOEStateMachine.PHASE_DEAD);
            String ovpStatus = event.get("POWER_SUPPLY_BAT_OVP", PPPOEStateMachine.PHASE_DEAD);
    ...
    private String mBatteryOvpStatus = PPPOEStateMachine.PHASE_DEAD;
    ...
    private String mQuickChargeStatus = PPPOEStateMachine.PHASE_DEAD;
    ...
    private void sendBatteryOvpErrorNotification() {
        ...
        String title = this.mContext.getResources().getString(33685897);
        CharSequence message = this.mContext.getResources().getString(33685898);
        ...
        intent.putExtra(OVP_ERR_NOTIFICATION_ID, 33685897);
        intent.setPackage(this.mContext.getPackageName());
        PendingIntent actionClickPI = PendingIntent.getBroadcast(this.mContext, 0, intent, 0);
        Notification.Builder build = new Notification.Builder(this.mContext).setSmallIcon(33751168).setOngoing(true).setContentTitle(title).setContentText(message).setContentIntent(pi).setTicker(title).setVibrate(new long[0]).setPriority(2).setWhen(System.currentTimeMillis()).setShowWhen(true).setVisibility(1).setAutoCancel(true);
        if (isChinaRegion) {
            build.addAction(new Action.Builder(null, this.mContext.getString(33685899), actionClickPI).build());
        }
        Notification notification = build.build();
        if (this.mNotificationManager != null) {
            this.mNotificationManager.notifyAsUser(null, 33685897, notification, UserHandle.ALL);
        }
    }

    private void handleBatteryOvpStatus(String status) {
        if (!this.mBatteryOvpStatus.equals(status) && PPPOEStateMachine.PHASE_INITIALIZE.equals(status)) {
    ...
    private String getQuickChargeBroadcastStatus(String fcpStatus, String scpStatus) {
        String status = PPPOEStateMachine.PHASE_DEAD;
        if (fcpStatus.equals(PPPOEStateMachine.PHASE_INITIALIZE) && scpStatus.equals(PPPOEStateMachine.PHASE_INITIALIZE)) {
            return "2";
        }
        if (fcpStatus.equals(PPPOEStateMachine.PHASE_INITIALIZE) && scpStatus.equals(PPPOEStateMachine.PHASE_DEAD)) {
            return PPPOEStateMachine.PHASE_INITIALIZE;
        }
        if (fcpStatus.equals(PPPOEStateMachine.PHASE_DEAD) && scpStatus.equals(PPPOEStateMachine.PHASE_INITIALIZE)) {
            return "2";
        }
        return status;
    }

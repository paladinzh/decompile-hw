...
public class HwCustPhoneWindowManagerImpl extends HwCustPhoneWindowManager {
    ...
    private static final boolean mSkipAuthKeyguard = SystemProperties.getBoolean("ro.config.skipAuthKeyguard", false);
    ...
    public boolean isAuthenticationGrace(Context context) {
        if (!mSkipAuthKeyguard) {
            return false;
        }
        boolean graceTest = false;
        FingerprintManager fpm = (FingerprintManager) context.getSystemService("fingerprint");
        if (fpm != null) {
            graceTest = fpm.isInGraceTime();
        }
        return graceTest;
    }

    public void skipAuthKeyguard(final KeyguardServiceDelegate keyguardDelegate, Handler handler) {
        if (keyguardDelegate.isShowing()) {
            handler.post(new Runnable() {
                public void run() {
                    keyguardDelegate.keyguardDone(false, false);
                }
            });
        }
    }

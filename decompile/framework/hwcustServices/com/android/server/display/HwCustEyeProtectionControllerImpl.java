        ...
        if (this.mNotificationManager != null) {
            if (this.mEyesProtectionMode == 1) {
                if (!this.mNotificationShown) {
                    ...
                    CharSequence title = r.getText(33685877);
                    CharSequence message = r.getText(33685878);
                    Notification notification = new Builder(this.mContext).setSmallIcon(33751077).setLargeIcon(BitmapFactory.decodeResource(r, 33751077)).setWhen(0).setOngoing(true).setTicker(title).setDefaults(0).setPriority(-2).setColor(this.mContext.getColor(17170519)).setContentTitle(title).setContentText(message).setContentIntent(PendingIntent.getActivityAsUser(this.mContext, 0, Intent.makeRestartActivityTask(new ComponentName("com.android.settings", "com.android.settings.EyeCareMainActivity")), 0, null, UserHandle.CURRENT)).setVisibility(1).build();
                    notification.extras = getNotificationThemeData(33751076, -1, 6, 15);
                    this.mNotificationShown = true;
                    this.mNotificationManager.notifyAsUser(null, 33685877, notification, UserHandle.ALL);
                }
            } else if (this.mNotificationShown) {
                this.mNotificationShown = false;
                this.mNotificationManager.cancelAsUser(null, 33685877, UserHandle.ALL);
            }
        }
    }

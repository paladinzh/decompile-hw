...
public class ConnectivityLogManager extends Handler {
    ...
    private static long mLastExLogsCheckTime = 0;
    private static final int mMaxExLogsCount = SystemProperties.getInt("ro.config.hw_chr_hi110x_cnt", 8);
    private static int mTotalChrTriggerCount = 0;
    public static final HashMap<Integer, CHRDataPlus> mapCHRDataPlus = new HashMap();
    private static ConnectivityLogManager sInstance;
    private boolean mIsConnected;
    private boolean mIsProcessing;
    private LocalSocket mLocalSocket = null;
    private LogException mLogException = HwFrameworkFactory.getLogException();
    private Runnable mProcessReadTimeout;
    private InputStream mReceiver;
    private OutputStream mSender;
    private boolean mWaitToDestroy = false;
    ...
    static {
        ...
        if (Log.HWINFO) {
            ...
            z = Log.isLoggable(LOG_TAG, 4);
        }
        HWFLOW = z;
    }

    public static void initCHRDataPlusMap() {
        mapCHRDataPlus.put(Integer.valueOf(80), null);
        mapCHRDataPlus.put(Integer.valueOf(81), null);
        mapCHRDataPlus.put(Integer.valueOf(84), null);
        mapCHRDataPlus.put(Integer.valueOf(85), null);
        mapCHRDataPlus.put(Integer.valueOf(86), null);
        mapCHRDataPlus.put(Integer.valueOf(87), null);
        mapCHRDataPlus.put(Integer.valueOf(82), null);
        mapCHRDataPlus.put(Integer.valueOf(83), null);
        mapCHRDataPlus.put(Integer.valueOf(101), null);
    ...
    private byte[] processCommand(String cmd) {
        if (this.mIsProcessing) {
            ...
            return sendCommand(cmd.getBytes(Charset.defaultCharset()));
        } else {
            return null;
        }
    }
    ...
    public byte[] sendCommand(byte[] cmd) {
        ...
        ByteBuffer response = ByteBuffer.wrap(new byte[8192]);
        int totalBytes = 0;
            ...
            if (dir.exists() || dir.mkdirs()) {
                ...
                try {
                    ...
                    while (true) {
                        ...
                        if (totalBytes > 0 || totalBytes > 8192) {
                            if (totalBytes <= 8192) {
                                Log.d(LOG_TAG, "response is too long, " + totalBytes + " > " + 8192);
                                return null;
                            }
                    ...
                    Log.e(LOG_TAG, "read bytes:" + (totalBytes + readBytes) + " more than max limit buf size:" + 8192);
                    totalBytes = 0;
                    response = null;
                    cancelTimerForRead();
                    close();
    ...
    private void getExInfo(ConnectivityLog connectivityLog) {
        ...
        if (connectivityLog.mEventType == 71 || connectivityLog.mEventType == GPS_SESSION_EVENT) {
    ...
    private void saveExInfo(String strJson) {
        ...
        if (str == null || -1 == eventNo || -1 == errorNo) {
            if (HWFLOW) {
        ...
        if (COUNTRY_CODE_CN.equalsIgnoreCase(SystemProperties.get("ro.product.locale.region", ""))) {
            ...
            ConnectivityLog connectivityLog = new ConnectivityLog(null, -1, -1, -1, null);
            if (str.compareTo(SUBSYS_WIFI) == 0) {
    ...
    private void reportConnectivityException(ConnectivityLog connectivityLog) {
        ...
        if (!LogManager.getInstance().isCommercialUser()) {
            ...
            model.chrLogComHeadModel = new ChrCommonInfo().getChrComHead(mContext, false);
            LogManager.getInstance().reportAbnormalEvent(model, connectivityLog.mMetricID, 1);
            int eventType = connectivityLog.mEventType;
            if (eventType != WIFI_STABILITY_STAT && eventType != 83) {
                if (eventType != 71 && eventType != GPS_SESSION_EVENT) {
    ...
    public void reportAbnormalEventEx(ChrLogBaseModel logModel, int metricID, int level, int eventType, Date date, int msg_type) {
        if (!LogManager.getInstance().isOverseaCommercialUser()) {
            if (logModel == null || metricID < 0 || level < 1) {
                Log.e(LOG_TAG, "illegal Parameter");
                return;
            }
            if (msg_type == 1) {
                Log.d(LOG_TAG, "reportAbnormalEventEx EX-BEGIN: normal type");
                sendMessage(logModel, metricID, level, eventType, date, false);
            } else {
    ...
    private void sendMessage(ChrLogBaseModel logModel, int metricID, int level, int eventType, Date date, boolean isDaily) {
        ...
        Message targetMsg = obtainMessage(1, new ConnectivityLog(logInfo, metricID, level, eventType, date));
        if (isDaily) {
    ...
    public boolean reportWifiExceptionLog(int type, String reason, Date time) {
        ...
        switch (type) {
            ...
            case WIFI_ABS_ASSOC_FAILED_EVENT /*215*/:
            case WIFI_ABS_STATISTICS_EVENT /*216*/:

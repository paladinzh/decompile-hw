...
public class HwLocalePickerManagerImpl implements HwLocalePickerManager {
        ...
        while (i < finalSize) {
            if (localeInfos[i].getLabel().equals(ap_str1) || localeInfos[i].getLabel().equals(ap_str2)) {
                if (!"".equals(SimOptr) && "294".equals(SimOptr)) {
                    ...
                    newlabel = ap_str1;
                } else if (SystemProperties.getInt("ro.config.hw_opta", 0) == 150 && SystemProperties.getInt("ro.config.hw_optb", 0) == 300) {
                    newlabel = ap_str1;
                } else {
                    newlabel = ap_str2;
                }
    ...
    private String getSimCountry(Context context, Configuration configuration) {
        ...
        try {
            ...
            return (String) clazz.getMethod("countryCodeForMcc", new Class[]{Integer.TYPE}).invoke(null, new Object[]{Integer.valueOf(configuration.mcc)});
        } catch (Exception e) {

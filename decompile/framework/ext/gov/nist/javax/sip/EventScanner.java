        ...
        if (sipEvent instanceof RequestEvent) {
            try {
                ...
                if (this.sipStack.isLoggingEnabled()) {
                    ...
                    stackLogger = stackLogger2;
                    stackLogger.logDebug("deliverEvent : " + sipRequest.getFirstLine() + " transaction " + eventWrapper.transaction + " sipEvent.serverTx = " + ((RequestEvent) sipEvent).getServerTransaction());
                }
                ...
                if (tx == null || tx.passToListener()) {
                    if (this.sipStack.findPendingTransaction(sipRequest) != null) {
                        ...
                        if (this.sipStack.isLoggingEnabled()) {
                            stackLogger2 = this.sipStack.getStackLogger();
                            stackLogger = stackLogger2;
                            stackLogger.logDebug("Done processing Message " + ((SIPRequest) ((RequestEvent) sipEvent).getRequest()).getFirstLine());
                        }
            ...
            try {
                if (this.sipStack.isLoggingEnabled()) {
                    ...
                    stackLogger2 = this.sipStack.getStackLogger();
                    stackLogger = stackLogger2;
                    stackLogger.logDebug("tx = " + ((TransactionTerminatedEvent) sipEvent).getClientTransaction());
                    stackLogger2 = this.sipStack.getStackLogger();
                    stackLogger = stackLogger2;
                    stackLogger.logDebug("tx = " + ((TransactionTerminatedEvent) sipEvent).getServerTransaction());
                }
                if (sipListener != null) {

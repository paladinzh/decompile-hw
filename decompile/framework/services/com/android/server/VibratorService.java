...
public class VibratorService extends Stub implements InputDeviceListener {
    ...
    private HwCustCbsUtils mCust = ((HwCustCbsUtils) HwCustUtils.createObj(HwCustCbsUtils.class, new Object[0]));
            ...
            if (intent.getAction().equals("android.intent.action.SCREEN_OFF")) {
                synchronized (VibratorService.this.mVibrations) {
                    if (!(VibratorService.this.mCurrentVibration == null || VibratorService.this.mCurrentVibration.isSystemHapticFeedback())) {
                        if (VibratorService.this.mCust == null || !VibratorService.this.mCust.isNotAllowPkg(VibratorService.this.mCurrentVibration.mOpPkg)) {
            ...
            try {
                if (pattern.length != 0) {
                    if (!(isAll0(pattern) || repeat >= pattern.length || token == null)) {
                        ...
                        Flog.i(NativeResponseCode.SERVICE_REGISTERED, "vibratePattern on: uid is " + uid + ",pid is " + Binder.getCallingPid() + ",packagename = " + packageName + ",repeat = " + repeat);
                        try {
    ...
    private int startVibrationLocked(Vibration vib) {
        Flog.i(NativeResponseCode.SERVICE_REGISTERED, "startVibrationLocked vib.getMode:" + vib.getMode() + ",vib.mTimeout:" + vib.mTimeout);
        try {

        ...
        if (displayContent.layoutNeeded) {
            ...
            while (i >= 0) {
                ...
                if (initial && IS_DEBUG_VERSION) {
                    ...
                    params.put("newCircle", Boolean.valueOf(i == N + -1));
                ...
                if (!win.isGoneForLayoutLw() && r13) {
                    gone = false;
                    if (!gone || !win.mHaveFrame || win.mLayoutNeeded || ((win.isConfigChanged() || win.setInsetsChanged()) && !win.isGoneForLayoutLw() && ((win.mAttrs.privateFlags & 1024) != 0 || (win.mHasSurface && win.mAppToken != null && win.mAppToken.layoutConfigChanges)))) {
                ...
                if (win.mAttrs.type == 2100 || win.mAttrs.type == 2101) {
                    ...
                    gone = win.mAttrs.type != IHwShutdownThread.SHUTDOWN_ANIMATION_WAIT_TIME;
                    if (win.mLayoutAttached) {
                        if (initial) {
                            win.mContentChanged = false;
                        }
    ...
    private boolean transitionGoodToGo(int appsCount) {
        ...
        while (i < appsCount) {
            ...
            if (wtoken.isRelaunching()) {
                Flog.i(307, "waiting for relaunch");
                return false;
            }
            boolean drawnBeforeRestoring = wtoken.allDrawn;
            wtoken.restoreSavedSurfaces();
            if (wtoken.hiddenRequested || wtoken.allDrawn || wtoken.startingDisplayed || wtoken.startingMoved) {
                if (!wtoken.allDrawn) {
                    reason = 1;
                ...
                Flog.i(307, "Delaying app transition only for visible window");
                return false;
            }
        }

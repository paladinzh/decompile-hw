...
*/
    ...
    private void writeTaskIdsFiles() {
        ...
        SparseBooleanArray taskIdsToSave = null;
        for (int userId : candidateUserIds) {
            synchronized (this.mService) {
                try {
                    ActivityManagerService.boostPriorityForLockedSection();
                    SparseBooleanArray temptaskIdsToSave = (SparseBooleanArray) this.mRecentTasks.mPersistedTaskIds.get(userId);
                    if (temptaskIdsToSave != null) {
                        taskIdsToSave = temptaskIdsToSave.clone();
                    }
                } catch (Throwable th) {
                    ActivityManagerService.resetPriorityAfterLockedSection();
                }

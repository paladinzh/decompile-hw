...
public class BroadcastQueue extends AbsBroadcastQueue {
    ...
    static {
        ...
        i = (ActivityManager.isLowRamDeviceStatic() || SystemProperties.getBoolean("ro.config.hw_low_ram", false)) ? 25 : 300;
    ...
    private final void processCurBroadcastLocked(BroadcastRecord r, ProcessRecord app) throws RemoteException {
        ...
        if (app.thread == null) {
            ...
            try {
                ...
                if (ActivityManagerDebugConfig.DEBUG_BROADCAST) {
                    Slog.v(TAG_BROADCAST, "Process cur broadcast " + r + " DELIVERED for app " + app + ", callerApp euid: " + (r.callerApp != null ? r.callerApp.info.euid : 0));
                }
                started = true;
            } finally {
                if (!started) {
                                ...
                                if (r != null) {
                                    ...
                                    if (enqueueProxyBroadcast(false, r, target)) {
                                        ...
                                        if (!this.mPendingBroadcastTimeoutMessage) {
                                            ...
                                            checkTime = r.receiverTime + JobStatus.DEFAULT_TRIGGER_UPDATE_DELAY;
                                            if (ActivityManagerDebugConfig.DEBUG_BROADCAST) {
        ...
        if (this.mOrderedBroadcasts.size() != 0) {
            ...
            if (fromMsg) {
                if (this.mService.mDidDexOpt) {
                    ...
                    long checkTime = r.receiverTime + JobStatus.DEFAULT_TRIGGER_UPDATE_DELAY;
                    if (checkTime > now) {
                        if (ActivityManagerDebugConfig.DEBUG_BROADCAST) {
                        ...
                        if (this.mService.isTopProcessLocked(((BroadcastRecord) this.mOrderedBroadcasts.get(0)).curApp)) {
                            ...
                            setBroadcastTimeoutLocked(now + JobStatus.DEFAULT_TRIGGER_UPDATE_DELAY);
                            return;
                        }
                    }
                } else {

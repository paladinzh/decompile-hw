...
class MagnificationController {
    ...
    private boolean updateMagnificationSpecLocked(float scale, float centerX, float centerY) {
        ...
        float offsetX = MathUtils.constrain(((((float) this.mMagnificationBounds.width()) / 2.0f) + ((float) this.mMagnificationBounds.left)) - (centerX * scale), getMinOffsetXLocked(), 0.0f);
        if (Float.compare(currSpec.offsetX, offsetX) != 0) {
            currSpec.offsetX = offsetX;
            changed = true;
        }
        float offsetY = MathUtils.constrain(((((float) this.mMagnificationBounds.height()) / 2.0f) + ((float) this.mMagnificationBounds.top)) - (centerY * scale), getMinOffsetYLocked(), 0.0f);

...
final class WifiDisplayController implements Dump {
    ...
    private final BroadcastReceiver mWifiP2pExReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            if ("android.net.wifi.p2p.WIFI_P2P_FLAG_CHANGED_ACTION".equals(intent.getAction()) && WifiDisplayControllerHisiExt.hisiWifiEnabled()) {
                boolean wifiP2pFlag = intent.getIntExtra("extra_p2p_flag", 0) == 1 ? WifiDisplayController.DEBUG : false;
    ...
    private final BroadcastReceiver mWifiP2pReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("android.net.wifi.p2p.STATE_CHANGED")) {
                boolean enabled = intent.getIntExtra("wifi_p2p_state", 1) == 2 ? WifiDisplayController.DEBUG : false;
    ...
    private void updateSettings() {
        boolean z = DEBUG;
        ContentResolver resolver = this.mContext.getContentResolver();
        this.mWifiDisplayOnSetting = Global.getInt(resolver, "wifi_display_on", 0) != 0 ? DEBUG : false;
                    ...
                    if (this.mConnectedDevice != null && this.mRemoteDisplay == null) {
                        ...
                        this.mRemoteDisplay = RemoteDisplay.listen(iface, new android.media.RemoteDisplay.Listener() {
                            public void onDisplayConnected(Surface surface, int width, int height, int flags, int session) {
                                if (WifiDisplayController.this.mConnectedDevice == oldDevice && !WifiDisplayController.this.mRemoteDisplayConnected) {
    ...
    private WifiDisplaySessionInfo getSessionInfo(WifiP2pGroup info, int session) {
        ...
        boolean z = info.getOwner().deviceAddress.equals(this.mThisDevice.deviceAddress) ? false : DEBUG;
    ...
    private static WifiDisplay createWifiDisplay(WifiP2pDevice device) {
        return new WifiDisplay(device.deviceAddress, device.deviceName, null, DEBUG, device.wfdInfo.isSessionAvailable(), false);
    }
}

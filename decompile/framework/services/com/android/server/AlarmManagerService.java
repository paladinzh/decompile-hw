...
public class AlarmManagerService extends AbsAlarmManagerService {
    ...
    private static final Intent NEXT_ALARM_CLOCK_CHANGED_INTENT = new Intent("android.app.action.NEXT_ALARM_CLOCK_CHANGED").addFlags(536870912);
    ...
    boolean mInteractive = RECORD_ALARMS_IN_HISTORY;
    InteractiveStateReceiver mInteractiveStateReceiver;
    private boolean mIsScreenOn = RECORD_ALARMS_IN_HISTORY;
    ...
    public static class Alarm {
        ...
        public Alarm(int _type, long _when, long _whenElapsed, long _windowLength, long _maxWhen, long _interval, PendingIntent _op, IAlarmListener _rec, String _listenerTag, WorkSource _ws, int _flags, AlarmClockInfo _info, int _uid, String _pkgName) {
            ...
            this.type = _type;
            this.origWhen = _when;
            boolean z = _type != 2 ? _type == 0 ? AlarmManagerService.RECORD_ALARMS_IN_HISTORY : false : AlarmManagerService.RECORD_ALARMS_IN_HISTORY;
            this.wakeup = z;
            this.when = _when;
        ...
        public void dump(PrintWriter pw, String prefix, long nowRTC, long nowELAPSED, SimpleDateFormat sdf) {
            boolean isRtc = (this.type == 1 || this.type == 0) ? AlarmManagerService.RECORD_ALARMS_IN_HISTORY : false;
            pw.print(prefix);
            pw.print("tag=");
            ...
            switch (msg.what) {
                ...
                case 4:
                    if (AlarmManagerService.this.mLocalDeviceIdleController != null) {
                        AlarmManagerService.this.mLocalDeviceIdleController.setAlarmsActive(msg.arg1 != 0 ? AlarmManagerService.RECORD_ALARMS_IN_HISTORY : false);
                        return;
                    }
                    return;
                case 6:
    ...
    public final class Batch {
        ...
        boolean canHold(long whenElapsed, long maxWhen) {
            return (this.end < whenElapsed || this.start > maxWhen) ? false : AlarmManagerService.RECORD_ALARMS_IN_HISTORY;
        }
    ...
    static {
        boolean isLoggable = !Log.HWINFO ? Log.HWModuleLog ? Log.isLoggable(TAG, 4) : false : RECORD_ALARMS_IN_HISTORY;
            ...
            for (i = 0; i < N; i++) {
                ...
                if ((alarm.flags & 4) != 0) {
                    ...
                    if (nowELAPSED < minTime) {
                        ...
                        setImplLocked(alarm, RECORD_ALARMS_IN_HISTORY, false);
                    }
                }
                alarm.count = 1;
                ...
                if (alarm.repeatInterval > 0) {
                    ...
                    setImplLocked(alarm.type, alarm.when + delta, nextElapsed, alarm.windowLength, maxTriggerTime(nowELAPSED, nextElapsed, alarm.repeatInterval), alarm.repeatInterval, alarm.operation, null, null, alarm.flags, RECORD_ALARMS_IN_HISTORY, alarm.workSource, alarm.alarmClock, alarm.uid, alarm.packageName);
                }
                if (alarm.wakeup) {
    ...
    void deliverAlarmsLocked(ArrayList<Alarm> triggerList, long nowELAPSED) {
        ...
        for (int i = 0; i < triggerList.size(); i++) {
            ...
            if (alarm == null) {
                ...
                boolean allowWhileIdle = (alarm.flags & 4) != 0 ? RECORD_ALARMS_IN_HISTORY : false;
                try {
                    String packageName;
    ...
    void setWakelockWorkSource(PendingIntent pi, WorkSource ws, int type, String tag, int knownUid, boolean first) {
        try {
            boolean unimportant = pi == this.mTimeTickSender ? RECORD_ALARMS_IN_HISTORY : false;
